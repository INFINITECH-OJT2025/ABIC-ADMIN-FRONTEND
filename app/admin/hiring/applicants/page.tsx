...
export default function ApplicantsForInterviewPage() {
  ...
  const [editingInitialId, setEditingInitialId] = useState<
    number | string | null
  >(null);
  const [editingFinalId, setEditingFinalId] = useState<number | string | null>(
    null,
  );
  ...
  const [initialNameErrors, setInitialNameErrors] = useState<
    Record<string, string>
  >({});
  const [savingInitialId, setSavingInitialId] = useState<
    number | string | null
  >(null);
  const [savingFinalId, setSavingFinalId] = useState<number | string | null>(
    null,
  );
  ...
  const loadData = async () => {
    ...
    const [
      ...
      fetch(`${apiUrl}/api/hiring/interviews?stage=initial`, {
        headers: { Accept: "application/json" },
      }),
      fetch(`${apiUrl}/api/hiring/interviews?stage=final`, {
        headers: { Accept: "application/json" },
      }),
      fetch(`${apiUrl}/api/hiring/interviews/final-candidates`, {
        headers: { Accept: "application/json" },
      }),
    ...
    const onboardedCountByPosition = onboardedRows.reduce<Map<string, number>>(
      (acc, row) => {
    ...
    const uniquePositions = Array.from<string>(
      new Set<string>(
        summaryRows
          .filter(
            (row: { position?: unknown; required_headcount?: unknown }) => {
              const position =
              ...
              const onboardedCount =
                onboardedCountByPosition.get(normalizePosition(position)) ?? 0;

              return requiredHeadcount - onboardedCount > 0;
            },
          )
          .map((row: { position?: unknown }) =>
            typeof row.position === "string" ? row.position.trim() : "",
          )
          .filter(Boolean),
      ),
    ...
    setInitialRows(
      Array.isArray(initialJson?.data) ? initialJson.data.map(toRow) : [],
    );
    setFinalRows(
      Array.isArray(finalJson?.data) ? finalJson.data.map(toRow) : [],
    );
    setFinalCandidates(
      Array.isArray(finalCandidatesJson?.data) ? finalCandidatesJson.data : [],
    );
  ...
  const addInitialRow = () => {
    ...
    if (hasIncompleteDraft) {
      toast.error(
        "Complete the current initial interview draft before adding another row.",
      );
      return;
    }
    ...
    setFinalRows((prev) =>
      prev.map((row) => {
        ...
        if (field === "initialInterviewId") {
          const candidate = finalCandidates.find(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const duplicateNamePattern =
          /name already exists|already exists in interviews|already exists in employee records/i;
  ...
  const isInitialEditable = (row: InterviewRow) =>
    row.isNew || editingInitialId === row.id;
  const isFinalEditable = (row: InterviewRow) =>
    row.isNew || editingFinalId === row.id;
      ...
      <div className="bg-gradient-to-r from-[#A4163A] to-[#7B0F2B] text-white shadow-md mb-6">
        <div className="w-full px-4 md:px-8 py-6">
          ...
          <p className="text-white/80 text-sm md:text-base">
            Manage interview applicants and schedules.
          </p>
          {isViewOnly && (
            ...
            <Table className="border-collapse w-full text-[12px] table-fixed">
              <TableHeader>
                <TableRow className="bg-[#FFE5EC]/30 sticky top-0 border-b border-[#FFE5EC] hover:bg-[#FFE5EC]/30">
                  <TableHead className="px-4 py-2 text-left font-bold text-[#800020] text-[10px] uppercase tracking-wider border-r border-[#FFE5EC]/50">
                    Applicant Name
                  </TableHead>
                  <TableHead className="px-4 py-2 text-left font-bold text-[#800020] text-[10px] uppercase tracking-wider border-r border-[#FFE5EC]/50">
                    Position
                  ...
                  <TableHead className="px-4 py-2 text-right font-bold text-[#800020] text-[10px] uppercase tracking-wider">
                    Action
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody className="divide-y divide-stone-100">
                {filteredInitialRows.map((row) => {
                  ...
                  const rowPositionOptions =
                    row.position && !positionOptions.includes(row.position)
                      ? [...positionOptions, row.position].sort((a, b) =>
                          a.localeCompare(b),
                        )
                      : positionOptions;
                  return (
                    ...
                    >
                      <TableCell className="px-4 py-2 relative border-r border-rose-50/40">
                        ...
                        <input
                          ...
                          readOnly={!editable}
                          onChange={(e) =>
                            handleInitialChange(row.id, "name", e.target.value)
                          }
                      ...
                      <TableCell className="px-4 py-2 border-r border-rose-50/40">
                        <input
                          ...
                          readOnly={!editable}
                          onChange={(e) =>
                            handleInitialChange(row.id, "date", e.target.value)
                          }
                      ...
                      <TableCell className="px-4 py-2 border-r border-rose-50/40">
                        <input
                          ...
                          readOnly={!editable}
                          onChange={(e) =>
                            handleInitialChange(row.id, "time", e.target.value)
                          }
                          className="w-full h-full bg-transparent border-none text-center font-bold text-black focus:outline-none read-only:opacity-80"
                        />
                      </TableCell>
              ...
              <TableBody className="divide-y divide-stone-100">
                {filteredFinalRows.map((row) => {
                  ...
                  return (
                    ...
                    >
                      <TableCell className="px-4 py-2 border-r border-rose-50/40">
                        <div className="w-full h-full px-4 flex items-center font-bold text-black">
                          {row.name || "-"}
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="px-4 py-2 border-r border-rose-50/40">
                        <input
                          ...
                          readOnly={!editable}
                          onChange={(e) =>
                            handleFinalChange(row.id, "date", e.target.value)
                          }
                          className="w-full h-full bg-transparent border-none text-center font-bold text-black focus:outline-none read-only:opacity-80"
                        />
                      </TableCell>
                      <TableCell className="px-4 py-2 border-r border-rose-50/40">
                        <input
                          ...
                          readOnly={!editable}
                          onChange={(e) =>
                            handleFinalChange(row.id, "time", e.target.value)
                          }
                          className="w-full h-full bg-transparent border-none text-center font-bold text-black focus:outline-none read-only:opacity-80"
                        />
                      </TableCell>
