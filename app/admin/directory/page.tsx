...
import {
  PageEmptyState,
  PageErrorState,
} from "@/components/state/page-feedback";
...
type CloudinaryPickerTarget =
  | { type: "agency"; agencyCode: string }
  | { type: "general-contact"; index: number };
...
const getDirectoryFolderPath = (sectionCode: string): string =>
  `backend/storage/uploads/images/${getDirectorySectionFolder(sectionCode)}`;
...
const getDetailIcon = (type: string, label: string) => {
  ...
  if (normalizedType.includes("mobile") || normalizedType.includes("hotline"))
    return Phone;
  if (normalizedLabel.includes("email")) return Mail;
  if (
    normalizedLabel.includes("website") ||
    normalizedLabel.includes("official page")
  )
    return Globe;
  if (normalizedLabel.includes("address") || normalizedLabel.includes("office"))
    return MapPin;
  return Phone;
};
...
export default function GovernmentDirectoryPage() {
  ...
  const [agenciesByCode, setAgenciesByCode] = useState<
    Record<string, BackendAgency>
  >({});
  const [updatingImage, setUpdatingImage] = useState(false);
  const [cloudinaryPickerOpen, setCloudinaryPickerOpen] = useState(false);
  const [cloudinaryPickerTarget, setCloudinaryPickerTarget] =
    useState<CloudinaryPickerTarget | null>(null);
  ...
  const [agencyImageRefreshKey, setAgencyImageRefreshKey] = useState<
    Record<string, number>
  >({});
  const [agencyImageRetryCount, setAgencyImageRetryCount] = useState<
    Record<string, number>
  >({});
  const [generalContacts, setGeneralContacts] = useState<GeneralContact[]>([]);
  const [generalContactsDraft, setGeneralContactsDraft] = useState<
    EditableGeneralContact[]
  >([]);
  ...
  const generalContactsTotalPages = useMemo(() => {
    const totalRows = editingGeneralContacts
      ? filteredGeneralContactsDraft.length
      : filteredGeneralContacts.length;
  ...
  const paginatedFilteredGeneralContacts = useMemo(() => {
    ...
    return filteredGeneralContacts.slice(
      start,
      start + GENERAL_CONTACTS_PAGE_SIZE,
    );
  }, [filteredGeneralContacts, generalContactsPage]);

  const paginatedFilteredGeneralContactsDraft = useMemo(() => {
    const start = (generalContactsPage - 1) * GENERAL_CONTACTS_PAGE_SIZE;
    return filteredGeneralContactsDraft.slice(
      start,
      start + GENERAL_CONTACTS_PAGE_SIZE,
    );
  ...
  const cloudinaryTotalPages = useMemo(() => {
    return Math.max(
      1,
      Math.ceil(cloudinaryImages.length / CLOUDINARY_PAGE_SIZE),
    );
  ...
  const cloudinaryPickerFolderPath = useMemo(() => {
    const sectionCode =
      cloudinaryPickerTarget?.type === "general-contact"
  ...
  const directoryQuery = useQuery({
    ...
    queryFn: async (): Promise<Record<string, BackendAgency>> => {
      ...
      await ensureOkResponse(
        response,
        "Unable to load directory data right now.",
      );
  ...
  const agency = useMemo(
    () =>
      mergedAgencies.find((item) => item.key === activeAgency) ??
      mergedAgencies[0] ??
      null,
  ...
  const currentSteps = useMemo(() => {
    ...
    const fromBackend = Array.isArray(backend?.processes)
      ? backend.processes
          .filter(
            (row) =>
              String(row.process_type || "").toLowerCase() ===
              activeProcess.toLowerCase(),
          )
  ...
  const activeBackendAgency = useMemo(
    () => agenciesByCode[activeAgency],
    [agenciesByCode, activeAgency],
  );
  ...
  const snapshot = useMemo(() => {
    ...
    const addingStepsCount =
      editMode && draft
        ? draft.processes.filter(
            (row) =>
              row.process_type === "Adding" && row.process.trim().length > 0,
          ).length
        : (activeBackendAgency?.processes?.filter(
            (row) => String(row.process_type).toLowerCase() === "adding",
          ).length ?? 0);

    const removingStepsCount =
      editMode && draft
        ? draft.processes.filter(
            (row) =>
              row.process_type === "Removing" && row.process.trim().length > 0,
          ).length
        : (activeBackendAgency?.processes?.filter(
            (row) => String(row.process_type).toLowerCase() === "removing",
          ).length ?? 0);

    const updatedAtMs = parseBackendDateMs(
      activeBackendAgency?.updated_at || null,
    );
    const updatedAtText = !Number.isNaN(updatedAtMs)
      ? new Date(updatedAtMs).toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
          year: "numeric",
        })
    ...
    const activeLink =
      OFFICIAL_PORTALS.find((p) => normalizeCode(p.code) === activeAgency)
        ?.url || "N/A";
  ...
  const startEditMode = () => {
    ...
    const processes = Array.isArray(backend.processes)
      ? [...backend.processes]
          ...
          .map((row) => ({
            id: row.id,
            process_type: (String(row.process_type).toLowerCase() === "removing"
              ? "Removing"
              : "Adding") as ProcessType,
            process: row.process || "",
  ...
  const updateContactAt = (
    index: number,
    field: keyof EditableContact,
    value: string,
  ) => {
    setDraft((prev) => {
      if (!prev) return prev;
      ...
      return {
        ...
        contacts: [
          ...prev.contacts,
          {
            ...
            label: "",
            value: "",
            sort_order: prev.contacts.length + 1,
          },
        ],
      };
    });
  ...
  const addProcessStep = () => {
    setDraft((prev) => {
      if (!prev) return prev;
      const count = prev.processes.filter(
        (row) => row.process_type === activeProcess,
      ).length;
      return {
        ...prev,
        processes: [
          ...prev.processes,
          { process_type: activeProcess, process: "", step_number: count + 1 },
        ],
      };
    });
  };
  ...
  const saveDirectoryChanges = async () => {
    ...
    try {
      ...
      if (!draftValidation.success) {
        const message =
          draftValidation.error.issues[0]?.message ||
          "Please review your directory entries.";
      ...
      if (duplicateContactIndices.size > 0 || duplicateProcessCount > 0) {
        toast.warning("Duplicate Entries Detected", {
          description:
            "Rows highlighted in red are duplicates. Resolve them before saving directory changes.",
        });
        return;
      }
      ...
      const processes = (["Adding", "Removing"] as ProcessType[]).flatMap(
        (type) => {
          const rows = draft.processes.filter(
            (row) => row.process_type === type && row.process.trim().length > 0,
          );
      ...
      const response = await fetch(
        `${getApiUrl()}/api/directory/agencies/${activeAgency}`,
        {
          method: "PUT",
          headers: {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to save directory changes";
  ...
  const persistAgencyImage = async (params: {
    ...
    if (!targetAgencyCode) {
      throw new Error(
        "Please select a valid agency before updating its picture.",
      );
    }

    const response = await fetch(
      `${getApiUrl()}/api/directory/agencies/${targetAgencyCode}/image`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
    ...
    setAgenciesByCode((prev) => {
      ...
      const merged: BackendAgency = {
        ...
        contacts: Array.isArray(updated?.contacts)
          ? (updated.contacts as BackendContact[])
          : fallback.contacts,
        processes: Array.isArray(updated?.processes)
          ? (updated.processes as BackendProcess[])
          : fallback.processes,
      };
  ...
  const handleAgencyImageError = (agencyCode: string) => {
    ...
    if (attempts < 3) {
      setAgencyImageRetryCount((prev) => ({
        ...prev,
        [agencyCode]: attempts + 1,
      }));
      setAgencyImageRefreshKey((prev) => ({
        ...prev,
        [agencyCode]: Date.now(),
      }));
      return;
  ...
  const loadCloudinaryImages = async (target: CloudinaryPickerTarget) => {
    try {
      ...
      await ensureOkResponse(
        response,
        "Unable to load uploaded images from backend storage.",
      );
      const result = await response.json();
      const rows = Array.isArray(result?.data) ? result.data : [];
  ...
  const loadGeneralContacts = async () => {
    try {
      ...
      const response = await fetch(
        `${getApiUrl()}/api/directory/general-contacts`,
        {
          headers: { Accept: "application/json" },
        },
      );
      ...
      const message =
        err instanceof Error ? err.message : "Failed to load general contacts";
  ...
  const validateImageRestrictions = (
    formatValue: string,
    bytesValue: number,
  ) => {
    ...
    if (
      !ALLOWED_UPLOAD_FORMATS.includes(
        format as (typeof ALLOWED_UPLOAD_FORMATS)[number],
      )
    ) {
      throw new Error(
        "Only JPEG/JPG, PNG, GIF, WebP, and HEIC/HEIF are allowed.",
      );
    }
  ...
  const setGeneralContactAvatarAt = (
    index: number,
    imageUrl: string,
    publicId: string,
  ) => {
  ...
  const uploadGeneralContactAvatar = async (index: number, file: File) => {
    try {
      ...
      const format =
        dotIndex >= 0 ? rawName.slice(dotIndex + 1).toLowerCase() : "";
  ...
  const handleGeneralContactUploadInputChange = (
  ...
  const saveGeneralContacts = async () => {
    try {
      const contactsValidation =
        generalContactsSchema.safeParse(generalContactsDraft);
      if (!contactsValidation.success) {
        const message =
          contactsValidation.error.issues[0]?.message ||
          "Please review all general contact fields.";
        toast.error("Validation Failed", {
          description: message,
        });
        return;
      ...
      if (duplicateGeneralContactIndices.size > 0) {
        toast.warning("Duplicate Entries Detected", {
          description:
            "Rows highlighted in red are duplicates. Resolve them before saving general contacts.",
        });
        return;
      }
      ...
      const message =
        err instanceof Error ? err.message : "Failed to save general contacts";
      toast.error("Save Failed", { description: message });
    } finally {
  ...
  const handleAgencyUploadInputChange = (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const targetAgencyCode = normalizeCode(
  ...
  return (
    <div className="min-h-screen bg-[#F5F6F8] font-sans flex flex-col">
                ...
                >
                  <Icon
                    className={cn(
                      ...
                      isActive ? "w-6 h-6 stroke-[3]" : "w-5 h-5 stroke-2",
                    )}
                  />
                  <span
                    className={cn(
        ...
        {isGeneralContactsView ? (
          <Card className="border border-slate-200 shadow-xl rounded-sm overflow-hidden">
            ...
            <div className="px-6 md:px-8 py-6">
              {loadingGeneralContacts ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {Array.from({ length: 8 }).map((_, idx) => (
                    <div
                      key={`general-contact-skeleton-${idx}`}
                ...
                <div className="space-y-3">
                  ...
                  {duplicateGeneralContactIndices.size > 0 && (
                    <div className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-[12px] font-bold text-rose-700 flex items-center gap-2">
                      ...
                      {duplicateGeneralContactIndices.size} duplicate contact
                      value
                      {duplicateGeneralContactIndices.size > 1
                        ? "s are"
                        : " is"}{" "}
                      highlighted in red. Resolve duplicates before saving.
                    </div>
                  )}
                  {generalContactsDraft.length === 0 ? (
                    ...
                    <>
                      {paginatedFilteredGeneralContactsDraft.map(
                        ({ row, index }) => {
                          const isDuplicateGeneralContact =
                            duplicateGeneralContactIndices.has(index);
                          return (
                            <div
                              ...
                              className={cn(
                                ...
                                isDuplicateGeneralContact
                                  ? "bg-rose-50/70 ring-1 ring-rose-300/90 border-rose-300"
                                  : "",
                              )}
                            >
                              {isDuplicateGeneralContact && (
                                ...
                                <div className="flex items-center gap-2 min-w-0">
                                  <div className="h-10 w-10 rounded-full overflow-hidden border border-slate-200 bg-slate-100 shrink-0 flex items-center justify-center">
                                    {row.avatar_url ? (
                                      <img
                                        src={row.avatar_url}
                                        alt={row.establishment_name || "Avatar"}
                                ...
                                <DropdownMenu>
                                  ...
                                  >
                                    ...
                                    <DropdownMenuItem
                                      onSelect={(e) => {
                                        e.preventDefault();
                                        openGeneralContactCloudinaryPicker(
                                          index,
                                        );
                                      }}
                                    >
                                    ...
                                    <DropdownMenuItem
                                      ...
                                      onSelect={(e) => {
                                        ...
                                        toast.success(
                                          "Avatar cleared. Save contacts to apply changes.",
                                        );
                                      }}
                                    >
                              ...
                              <TextFieldStatus
                                value={row.value}
                                min={
                                  VALIDATION_CONSTRAINTS.directory.generalValue
                                    .min
                                }
                                max={
                                  VALIDATION_CONSTRAINTS.directory.generalValue
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {paginatedFilteredGeneralContacts.map((contact) => {
                    return (
                      <div
                        key={contact.id}
                        className="flex items-start gap-3 rounded-md border border-slate-100 p-3 min-w-0"
                      >
                        {contact.avatar_url ? (
              ...
              {!loadingGeneralContacts &&
              (editingGeneralContacts
                ? filteredGeneralContactsDraft.length > 0
                : filteredGeneralContacts.length > 0) ? (
                <div className="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-4 md:flex-row md:items-center md:justify-between">
                  <p className="text-xs font-semibold text-slate-500">
                    Showing{" "}
                    {(generalContactsPage - 1) * GENERAL_CONTACTS_PAGE_SIZE + 1}
                    -
                    {(generalContactsPage - 1) * GENERAL_CONTACTS_PAGE_SIZE +
                      (editingGeneralContacts
                        ? paginatedFilteredGeneralContactsDraft.length
                        : paginatedFilteredGeneralContacts.length)}{" "}
                    of{" "}
                    {editingGeneralContacts
                      ? filteredGeneralContactsDraft.length
                      : filteredGeneralContacts.length}
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      onClick={() =>
                        setGeneralContactsPage((prev) => Math.max(1, prev - 1))
                      }
                    >
                    ...
                    <Button
                      ...
                      className="h-8 px-3"
                      disabled={
                        generalContactsPage >= generalContactsTotalPages
                      }
                      onClick={() =>
                        setGeneralContactsPage((prev) =>
                          Math.min(generalContactsTotalPages, prev + 1),
                        )
                      }
                    >
                      Next
                  ...
                  {editMode && draft ? (
                    <div className="space-y-3">
                      <Input
                        ...
                        maxLength={
                          VALIDATION_CONSTRAINTS.directory.agencyName.max
                        }
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              ...
                              isActive
                                ? "bg-[#A4163A] text-white shadow-md transform scale-105"
                ...
                <div className="relative z-10">
                  {editMode && duplicateProcessCount > 0 && (
                    <div className="mb-3 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-[12px] font-bold text-rose-700 flex items-center gap-2">
                      <TriangleAlert className="h-4 w-4 shrink-0" />
                      {duplicateProcessCount} duplicate process step
                      {duplicateProcessCount > 1 ? "s are" : " is"} highlighted
                      in red. Resolve duplicates before saving.
                    </div>
                  )}
                    ...
                    <Table>
                      <TableHeader className="bg-slate-50/50">
                        <TableRow className="hover:bg-slate-50/50 border-slate-100">
                          <TableHead className="w-[80px] text-center font-black text-slate-400 uppercase text-[20px] tracking-widest py-4">
                            Step
                          </TableHead>
                          <TableHead className="font-black text-slate-400 uppercase text-[20px] tracking-widest py-4">
                            Process Description
                          </TableHead>
                        </TableRow>
                      </TableHeader>
                        ...
                        {editMode ? (
                          currentProcessDraftRows.length > 0 ? (
                            currentProcessDraftRows.map((step, index) => {
                              const normalizedProcess = normalizeDuplicateValue(
                                step.process,
                              );
                              const isDuplicateProcess =
                                normalizedProcess.length > 0 &&
                                duplicateProcessKeys.has(
                                  `${step.process_type}|${normalizedProcess}`,
                                );
                              return (
                                <TableRow
                                  key={index}
                                  className={cn(
                                    ...
                                    isDuplicateProcess
                                      ? "bg-rose-50/70 ring-1 ring-rose-300/90"
                                      : "",
                                  )}
                                >
                          ...
                          currentSteps.map((step, index) => (
                            ...
                            >
                              ...
                              <TableCell className="py-6 align-top">
                                <p className="text-xl font-medium text-slate-800 leading-relaxed pt-1.5">
                                  {step}
                                </p>
                              </TableCell>
                            </TableRow>
                          ...
                          <TableRow>
                            <TableCell
                              ...
                              className="h-32 text-center text-slate-400 italic"
                            >
                              No process steps found for this category.
                            </TableCell>
                          </TableRow>
                        )}
                      </TableBody>
            ...
            <div>
              ...
              {editMode && duplicateContactIndices.size > 0 && (
                <div className="mb-4 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-[12px] font-bold text-rose-700 flex items-center gap-2">
                  <TriangleAlert className="h-4 w-4 shrink-0" />
                  {duplicateContactIndices.size} duplicate contact value
                  {duplicateContactIndices.size > 1 ? "s are" : " is"}{" "}
                  highlighted in red. Resolve duplicates before saving.
                </div>
              )}

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {(editMode && draft ? draft.contacts : agency.details)
                  .filter((row) => {
                  ...
                  .map((row, idx) => {
                    ...
                    const isDuplicateContact =
                      isEditable && duplicateContactIndices.has(idx);
                        ...
                        {isEditable ? (
                          <div className="flex-1 space-y-2 relative z-10 w-full">
                            ...
                            <TextFieldStatus
                              value={row.type}
                              max={
                                VALIDATION_CONSTRAINTS.directory.contactType.max
                              }
                            />
                            <TextFieldStatus
                              value={row.label}
          ...
          {!isGeneralContactsView ? (
            <div className="flex items-center gap-2">
              ...
              {snapshot.activeLink !== "N/A" ? (
                <a
                  href={snapshot.activeLink}
                  target="_blank"
                  rel="noreferrer"
                  className="hover:underline hover:text-white text-white/90"
                >
                  {snapshot.activeLink}
                </a>
              ) : (
      ...
      >
        <DialogContent className="sm:max-w-md p-0 overflow-hidden">
          <DialogHeader className="px-6 pt-6">
            <DialogTitle>
              {avatarPreview?.title || "Avatar Preview"}
            </DialogTitle>
            <DialogDescription>
              General contact avatar preview.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog
        open={loadingCloudinaryImages && !cloudinaryPickerOpen}
        onOpenChange={() => {}}
      >
        <DialogContent className="sm:max-w-md p-0 overflow-hidden">
          <DialogHeader className="px-6 pt-6">
              ...
              <>
                      ...
                      onClick={async () => {
                        try {
                          ...
                          if (
                            ...
                            setGeneralContactAvatarAt(
                              cloudinaryPickerTarget.index,
                              asset.secure_url,
                              asset.public_id,
                            );
                            toast.success(
                              "Avatar selected. Save contacts to apply changes.",
                            );
                          } else {
                            await persistAgencyImage({
                            ...
                            toast.success(
                              "Agency picture updated successfully!",
                            );
                          }
                    ...
                    >
                      ...
                      <div className="p-3">
                        ...
                        <p className="text-[11px] text-slate-500 mt-1">
                          ...
                          {Math.round(
                            (Number(asset.bytes || 0) / (1024 * 1024)) * 10,
                          ) / 10}{" "}
                          MB
                        </p>
                      </div>
                ...
                <div className="mt-4 flex items-center justify-between gap-3">
                  <p className="text-xs text-slate-500">
                    Page {cloudinaryPage} of {cloudinaryTotalPages} •{" "}
                    {cloudinaryImages.length} uploaded image
                    {cloudinaryImages.length === 1 ? "" : "s"}
                  </p>
                  <div className="flex items-center gap-2">
                    <Button
                      type="button"
                      ...
                      onClick={() =>
                        setCloudinaryPage((prev) => Math.max(1, prev - 1))
                      }
                    >
                      Previous
                    </Button>
                    <Button
                      ...
                      onClick={() =>
                        setCloudinaryPage((prev) =>
                          Math.min(cloudinaryTotalPages, prev + 1),
                        )
                      }
                    >
                      Next
        ...
        >
          <AlertDialogHeader>
            ...
            <AlertDialogTitle
              ...
              tabIndex={-1}
              className={modalTokens.title}
            >
              Delete this uploaded image?
            </AlertDialogTitle>
            <AlertDialogDescription className={modalTokens.description}>
              This will permanently delete the selected image file from backend
              storage.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className={modalTokens.footer}>
            <AlertDialogCancel
              ...
              className={buttonTokens.neutral}
            >
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const response = await fetch(
                    `${getApiUrl()}/api/directory/images`,
                    {
                      method: "DELETE",
                      headers: {
                        "Content-Type": "application/json",
                      ...
                      body: JSON.stringify({
                        public_id: deleteCandidate.public_id,
                      }),
                    },
                  );
                  ...
                  setCloudinaryImages((prev) =>
                    prev.filter(
                      (item) => item.public_id !== deleteCandidate.public_id,
                    ),
                  );
