...
function normalizeType(value: string) {
  ...
  if (
    ...
    return "forms";
  if (type.includes("hierarch") || type.includes("department"))
    return "hierarchy";
  if (type.includes("hiring")) return "hiring";
  if (
    type.includes("inventory") ||
    type.includes("office_supply") ||
    type.includes("office-supply")
  )
    return "inventory";
  return type;
}
...
export default function AdminHeadActivityLogsPage() {
  ...
  const [workingAction, setWorkingAction] = useState<
    "refresh" | "read" | "delete" | "single-read" | null
  >(null);
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        "/api/laravel/api/activity-logs?per_page=200",
        {
          method: "GET",
        },
      );
  ...
  const emitUnreadCount = useCallback((nextLogs: ActivityLogRow[]) => {
    const count = nextLogs.reduce(
      (acc, row) => (row.read_at ? acc : acc + 1),
      0,
    );
    window.dispatchEvent(
      new CustomEvent("activity-log-unread-changed", { detail: { count } }),
    );
  }, []);
  ...
  const markAllAsRead = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        "/api/laravel/api/activity-logs/mark-all-read",
        {
          method: "POST",
        },
      );
  ...
  const markSingleAsRead = useCallback(
    async (id: number) => {
      ...
      try {
        const response = await fetch(
          `/api/laravel/api/activity-logs/${id}/mark-read`,
          {
            method: "PATCH",
          },
        );
        ...
        const nextLogs = logs.map((row) =>
          row.id === id ? { ...row, read_at: row.read_at || nowIso } : row,
        );
        setLogs(nextLogs);
        emitUnreadCount(nextLogs);
      } catch (err) {
  ...
  const moduleCounts = useMemo(() => {
    return MODULE_TYPES.reduce<Record<string, number>>((acc, type) => {
      acc[type] = logs.filter(
        (row) => normalizeType(row.activity_type) === type,
      ).length;
      return acc;
    }, {});
  }, [logs]);
  ...
  const filteredLogs = useMemo(() => {
    ...
    if (typeFilter !== "all")
      rows = rows.filter(
        (row) => normalizeType(row.activity_type) === typeFilter,
      );
    if (statusFilter !== "all")
      rows = rows.filter((row) => row.status === statusFilter);
    if (readFilter === "read")
      rows = rows.filter((row) => Boolean(row.read_at));
  ...
  const selectedLog = useMemo(
    () => logs.find((row) => row.id === selectedLogId) ?? null,
    [logs, selectedLogId],
  );
  ...
  if (selectedLog) {
    const selectedStatusClass =
      statusClassMap[selectedLog.status] || statusClassMap.info;
      ...
      <section className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-12">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mb-2">
                  <div className="p-3 bg-white/15 rounded-2xl backdrop-blur-md border border-white/20">
                    {DetailsIcon ? (
                      <DetailsIcon className="h-8 w-8 text-white" />
                    ) : (
                  ...
                  <div>
                    ...
                    <p className="text-white/70 text-sm font-bold uppercase tracking-widest mt-1">
                      Event Reference #{selectedLog.id}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="mx-auto max-w-[1000px] px-4 -mt-6 relative z-20">
          <div className="rounded-3xl border border-slate-200 bg-white p-8 md:p-10 shadow-2xl space-y-10">
            <div className="flex flex-wrap items-center gap-3">
              <span
                className={`rounded-full px-4 py-1.5 text-[10px] font-black uppercase tracking-widest border transition-colors ${selectedStatusClass} border-current/10`}
              >
                {selectedLog.status}
              </span>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="group rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition-all hover:border-[#9d1238]/20 hover:shadow-md">
                ...
                <p className="text-lg font-bold text-slate-900">
                  {selectedLog.read_at
                    ? `Viewed at ${formatDate(selectedLog.read_at)}`
                    : "Unread by Admin"}
                </p>
              ...
              <div className="group rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition-all hover:border-[#9d1238]/20 hover:shadow-md">
                ...
                <p className="text-lg font-bold text-slate-900">
                  {selectedLog.user_name || "System Automated"}
                </p>
                <p className="text-xs font-medium text-slate-500 mt-1 truncate">
                  {selectedLog.user_email || "no-reply@system.log"}
                </p>
              </div>
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-white/80 text-sm md:text-base">
                Track every insert, update, delete, terminate, and all system
                actions.
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                <RefreshCw
                  className={`h-4 w-4 ${workingAction === "refresh" ? "animate-spin" : ""}`}
                />
                REFRESH
              </button>
              <button
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex flex-wrap items-center bg-black/20 p-1 rounded-xl border border-white/10 gap-1">
                  ...
                  >
                    <span>All</span>
                    <span
                      className={`px-1.5 py-0.5 rounded-md text-[10px] ${typeFilter === "all" ? "bg-[#9d1238]/10 text-[#9d1238]" : "bg-white/10 text-white/50"}`}
                    >
                      {logs.length}
                    </span>
                  </button>
                  {MODULE_TYPES.map((module) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span>{module}</span>
                        <span
                          className={`px-1.5 py-0.5 rounded-md text-[10px] ${typeFilter === module ? "bg-[#9d1238]/10 text-[#9d1238]" : "bg-white/10 text-white/50"}`}
                        >
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <div className="flex items-center bg-black/20 p-1 rounded-xl border border-white/10 gap-1">
                    ...
                    >
                      UNREAD
                      <span
                        className={`px-1.5 py-0.5 rounded-md text-[10px] ${readFilter === "unread" ? "bg-[#9d1238]/10 text-[#9d1238]" : "bg-white/10 text-white/50"}`}
                      >
                        {unreadCount}
                      </span>
                    </button>
      ...
      <div className="mx-auto max-w-[1200px] px-4 pb-8 md:px-8 md:pb-12">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {visibleLogs.map((log) => {
              const statusClass =
                statusClassMap[log.status] || statusClassMap.info;
              return (
                <article
                  key={log.id}
                  ...
                  <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-2 mb-1.5">
                        <h2
                          className={`text-lg font-bold transition-colors ${!log.read_at ? "text-[#9d1238]" : "text-slate-800 group-hover:text-[#9d1238]"}`}
                        >
                          {log.title}
                        </h2>
                        {!log.read_at && (
                    ...
                    <div className="flex flex-wrap items-center gap-2 md:justify-end shrink-0">
                      <span
                        className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[10px] font-black uppercase tracking-widest border transition-colors ${log.read_at ? "bg-slate-50 text-slate-500 border-slate-100" : "bg-blue-50 text-blue-600 border-blue-100"}`}
                      >
                        <Eye className="h-3.5 w-3.5" />
                        {log.read_at ? "Viewed" : "Unread"}
                      </span>
                      <span
                        className={`rounded-full px-3 py-1.5 text-[10px] font-black uppercase tracking-widest border ${statusClass} border-current/10`}
                      >
                        {log.status}
                      </span>
                      <span className="rounded-full bg-slate-100 border border-slate-200 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-slate-600 flex items-center gap-1">
                        {(() => {
                          const LogIcon =
                            MODULE_ICONS[normalizeType(log.activity_type)];
                          return LogIcon ? (
            ...
            {hasMore ? (
              <div className="pt-4 flex justify-center">
                <button
                  type="button"
                  onClick={() =>
                    setVisibleCount((current) => current + PAGE_STEP)
                  }
