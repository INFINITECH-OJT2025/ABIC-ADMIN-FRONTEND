...
const DEFAULT_DEPARTMENT_COLOR_TONE: DepartmentColorTone = {
  badge:
    "bg-slate-100 text-slate-700 border-slate-300 hover:bg-slate-100 hover:text-slate-700 hover:border-slate-300 group-data-[selected=true]:bg-slate-100 group-data-[selected=true]:text-slate-700 group-data-[selected=true]:border-slate-300",
  swatch: "bg-slate-300 border-slate-400",
};
const DEPARTMENT_COLOR_TONES: DepartmentColorTone[] = [
  {
    badge:
      "bg-violet-100 text-violet-800 border-violet-300 hover:bg-violet-100 hover:text-violet-800 hover:border-violet-300 group-data-[selected=true]:bg-violet-100 group-data-[selected=true]:text-violet-800 group-data-[selected=true]:border-violet-300",
    swatch: "bg-violet-400 border-violet-500",
  },
  {
    badge:
      "bg-fuchsia-100 text-fuchsia-800 border-fuchsia-300 hover:bg-fuchsia-100 hover:text-fuchsia-800 hover:border-fuchsia-300 group-data-[selected=true]:bg-fuchsia-100 group-data-[selected=true]:text-fuchsia-800 group-data-[selected=true]:border-fuchsia-300",
    swatch: "bg-fuchsia-400 border-fuchsia-500",
  },
  {
    badge:
      "bg-indigo-100 text-indigo-800 border-indigo-300 hover:bg-indigo-100 hover:text-indigo-800 hover:border-indigo-300 group-data-[selected=true]:bg-indigo-100 group-data-[selected=true]:text-indigo-800 group-data-[selected=true]:border-indigo-300",
    swatch: "bg-indigo-400 border-indigo-500",
  },
  {
    badge:
      "bg-blue-100 text-blue-800 border-blue-300 hover:bg-blue-100 hover:text-blue-800 hover:border-blue-300 group-data-[selected=true]:bg-blue-100 group-data-[selected=true]:text-blue-800 group-data-[selected=true]:border-blue-300",
    swatch: "bg-blue-400 border-blue-500",
  },
  {
    badge:
      "bg-cyan-100 text-cyan-800 border-cyan-300 hover:bg-cyan-100 hover:text-cyan-800 hover:border-cyan-300 group-data-[selected=true]:bg-cyan-100 group-data-[selected=true]:text-cyan-800 group-data-[selected=true]:border-cyan-300",
    swatch: "bg-cyan-400 border-cyan-500",
  },
  {
    badge:
      "bg-teal-100 text-teal-800 border-teal-300 hover:bg-teal-100 hover:text-teal-800 hover:border-teal-300 group-data-[selected=true]:bg-teal-100 group-data-[selected=true]:text-teal-800 group-data-[selected=true]:border-teal-300",
    swatch: "bg-teal-400 border-teal-500",
  },
  {
    badge:
      "bg-emerald-100 text-emerald-800 border-emerald-300 hover:bg-emerald-100 hover:text-emerald-800 hover:border-emerald-300 group-data-[selected=true]:bg-emerald-100 group-data-[selected=true]:text-emerald-800 group-data-[selected=true]:border-emerald-300",
    swatch: "bg-emerald-400 border-emerald-500",
  },
  {
    badge:
      "bg-lime-100 text-lime-800 border-lime-300 hover:bg-lime-100 hover:text-lime-800 hover:border-lime-300 group-data-[selected=true]:bg-lime-100 group-data-[selected=true]:text-lime-800 group-data-[selected=true]:border-lime-300",
    swatch: "bg-lime-400 border-lime-500",
  },
  {
    badge:
      "bg-yellow-100 text-yellow-800 border-yellow-300 hover:bg-yellow-100 hover:text-yellow-800 hover:border-yellow-300 group-data-[selected=true]:bg-yellow-100 group-data-[selected=true]:text-yellow-800 group-data-[selected=true]:border-yellow-300",
    swatch: "bg-yellow-400 border-yellow-500",
  },
  {
    badge:
      "bg-amber-100 text-amber-800 border-amber-300 hover:bg-amber-100 hover:text-amber-800 hover:border-amber-300 group-data-[selected=true]:bg-amber-100 group-data-[selected=true]:text-amber-800 group-data-[selected=true]:border-amber-300",
    swatch: "bg-amber-400 border-amber-500",
  },
  {
    badge:
      "bg-orange-100 text-orange-800 border-orange-300 hover:bg-orange-100 hover:text-orange-800 hover:border-orange-300 group-data-[selected=true]:bg-orange-100 group-data-[selected=true]:text-orange-800 group-data-[selected=true]:border-orange-300",
    swatch: "bg-orange-400 border-orange-500",
  },
  {
    badge:
      "bg-rose-100 text-rose-800 border-rose-300 hover:bg-rose-100 hover:text-rose-800 hover:border-rose-300 group-data-[selected=true]:bg-rose-100 group-data-[selected=true]:text-rose-800 group-data-[selected=true]:border-rose-300",
...
const getEmployeeDepartmentLabel = (
  employee: Pick<EmployeeRow, "department" | "position"> | null | undefined,
): string => {
  const department = String(employee?.department || "").trim();
  if (department) return department;
  const position = String(employee?.position || "").trim();
  return position
    ? `${NO_DEPARTMENT_ASSIGNED_LABEL} (${position})`
    : NO_DEPARTMENT_ASSIGNED_LABEL;
};

const getTransactionDepartmentLabel = (
  row: Pick<TransactionRow, "department_name" | "requested_by_position">,
): string => {
  const department = String(row.department_name || "").trim();
  const isNoDepartmentAssigned =
    department.toLowerCase() === NO_DEPARTMENT_ASSIGNED_LABEL.toLowerCase();
...
const getStockBadgeTone = (stock: number): string => {
  if (stock <= 0)
    return "bg-red-100 text-red-700 border-red-200 hover:bg-red-100 hover:text-red-700 hover:border-red-200 group-data-[selected=true]:bg-red-100 group-data-[selected=true]:text-red-700 group-data-[selected=true]:border-red-200";
  if (stock <= 10)
    return "bg-yellow-100 text-yellow-700 border-yellow-200 hover:bg-yellow-100 hover:text-yellow-700 hover:border-yellow-200 group-data-[selected=true]:bg-yellow-100 group-data-[selected=true]:text-yellow-700 group-data-[selected=true]:border-yellow-200";
...
const sanitizeIntegerInput = (value: string): string =>
  value.replace(/[^\d]/g, "");
const normalizeUppercaseInventoryText = (value: string): string =>
  value.toUpperCase();
...
export default function InventoryPage() {
  ...
  const [createItemFxStage, setCreateItemFxStage] =
    useState<CreateItemFxStage>("idle");
  const [createItemFxError, setCreateItemFxError] = useState<string | null>(
    null,
  );
  const [savingTransaction, setSavingTransaction] = useState(false);
  const [transactionFxOpen, setTransactionFxOpen] = useState(false);
  const [transactionFxStage, setTransactionFxStage] =
    useState<TransactionFxStage>("idle");
  const [transactionFxError, setTransactionFxError] = useState<string | null>(
    null,
  );
  const [transactionFxPayload, setTransactionFxPayload] = useState<{
    mode: QuantityFxMode;
    amount: number;
  } | null>(null);
  const [quantityChangeFx, setQuantityChangeFx] =
    useState<QuantityChangeFxState>({
      active: false,
      mode: "in",
      amount: 0,
      key: 0,
  ...
  const [createItemConfirmDraft, setCreateItemConfirmDraft] =
    useState<CreateItemConfirmDraft | null>(null);
  const [itemEditMode, setItemEditMode] = useState(false);
  const [editingItemId, setEditingItemId] = useState<number | null>(null);
  const [itemEditDraft, setItemEditDraft] =
    useState<InventoryItemEditDraft | null>(null);
  const [selectedItemIds, setSelectedItemIds] = useState<number[]>([]);
  const [itemIdsToDelete, setItemIdsToDelete] = useState<number[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState("");
  const [stockFilter, setStockFilter] = useState<
    "all" | "no-stock" | "low-stock" | "high-stock"
  >("all");
  ...
  const [transactionDateMode, setTransactionDateMode] =
    useState<TransactionDateMode>("year");
  const [transactionTypeFilter, setTransactionTypeFilter] = useState<
    "all" | "in" | "out"
  >("all");
  const [transactionDepartmentFilter, setTransactionDepartmentFilter] =
    useState("all");
  ...
  const [visualizationTopLimit, setVisualizationTopLimit] =
    useState<VisualizationTopLimit>("5");
  ...
  const [hoveredTopItemKey, setHoveredTopItemKey] = useState<string | null>(
    null,
  );
  const [hoveredDepartmentLabel, setHoveredDepartmentLabel] = useState<
    string | null
  >(null);
  ...
  const adminSupervisorHrEmployee = useMemo(() => {
    return (
      employees.find(
        (employee) =>
          normalizePosition(employee.position) === "admin supervisor/hr",
      ) ?? null
    );
  }, [employees]);

  const effectiveRequestedByEmployeeId =
    movementType === "in"
  ...
  const selectedItem = useMemo(
    () =>
      items.find((item) => String(item.id) === transactionDraft.item_id) ??
      null,
  ...
  const selectedEmployee = useMemo(
    () =>
      employees.find(
        (employee) => employee.id === effectiveRequestedByEmployeeId,
      ) ?? null,
  ...
  const duplicateItemNameMatch = useMemo(() => {
    ...
    return (
      items.find(
        (item) => normalizeForDuplicateCheck(item.item_name) === draftNameRaw,
      ) ?? null
    );
  ...
  const groupedEmployeesByDepartment = useMemo(() => {
    ...
    const sortedEmployees = [...employees].sort((a, b) => {
      ...
      return getEmployeeDisplayName(a)
        .toLowerCase()
        .localeCompare(getEmployeeDisplayName(b).toLowerCase());
    });
    ...
    return Array.from(grouped.entries()).map(([department, rows]) => ({
      department,
      rows,
    }));
  }, [employees]);
    ...
    return groupedEmployeesByDepartment.map((group) => {
      ...
      if (
        ...
        normalized.startsWith(
          normalizeDepartmentKey(NO_DEPARTMENT_ASSIGNED_LABEL),
        )
      ) {
        return {
          ...group,
      ...
      const tone =
        DEPARTMENT_COLOR_TONES[paletteIndex % DEPARTMENT_COLOR_TONES.length];
      paletteIndex += 1;
      return {
        ...group,
        normalized,
  ...
  const adminDepartment = useMemo(() => {
    const normalized = departments.map((department) => ({
      row: department,
      normalizedName: String(department.name || "")
        .trim()
        .toLowerCase(),
    }));
    ...
    return (
      normalized.find((entry) => entry.normalizedName.includes("admin"))?.row ??
      null
    );
  }, [departments]);
  ...
  const filteredItems = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      const left = String(a.item_name || "")
        .trim()
        .toLowerCase();
      const right = String(b.item_name || "")
        .trim()
        .toLowerCase();
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter((row) => {
      ...
      const matchesDateMode = (() => {
        if (transactionDateMode === "date") {
          return transactionDateFilter
            ? localDate === transactionDateFilter
            : true;
        }
        if (transactionDateMode === "month") {
          const matchesMonth =
            transactionMonthFilter === "all"
              ? true
              : monthValue === transactionMonthFilter;
      ...
      const matchesDepartment =
        transactionDepartmentFilter === "all"
          ? true
          : getTransactionDepartmentLabel(row).toLowerCase() ===
            transactionDepartmentFilter.toLowerCase();
  ...
  const topItemsByQuantityOut = useMemo(() => {
    const totals = new Map<
      string,
      { key: string; label: string; quantityOut: number }
    >();
    filteredTransactions.forEach((row) => {
      ...
      const current = totals.get(key) ?? {
        ...
        label:
          itemCode && itemName
            ? `${itemCode} - ${itemName}`
            : itemCode || itemName || "Unknown Item",
        quantityOut: 0,
      };
  ...
  const topItemDepartmentBreakdown = useMemo(() => {
    ...
    filteredTransactions.forEach((row) => {
      ...
      const itemBreakdown =
        breakdownByItem.get(itemKey) ?? new Map<string, number>();
      itemBreakdown.set(
        departmentLabel,
        (itemBreakdown.get(departmentLabel) ?? 0) + quantityOut,
      );
      breakdownByItem.set(itemKey, itemBreakdown);
    });

    return topItemsByQuantityOut.reduce(
      (acc, row) => {
        const entries = Array.from(
          (breakdownByItem.get(row.key) ?? new Map<string, number>()).entries(),
        )
  ...
  const departmentItemBreakdown = useMemo(() => {
    ...
    filteredTransactions.forEach((row) => {
      ...
      const departmentBreakdown =
        breakdownByDepartment.get(departmentLabel) ?? new Map<string, number>();
      departmentBreakdown.set(
        itemLabel,
        (departmentBreakdown.get(itemLabel) ?? 0) + quantityOut,
      );
      breakdownByDepartment.set(departmentLabel, departmentBreakdown);
    });

    return departmentOutData.reduce(
      (acc, row) => {
        const entries = Array.from(
          (
            breakdownByDepartment.get(row.label) ?? new Map<string, number>()
          ).entries(),
        )
  ...
  const visualizationAnimationKey = useMemo(() => {
    return [
      ...
      topItemsByQuantityOut
        .map((row) => `${row.key}:${row.quantityOut}`)
        .join("|"),
      departmentOutData
        .map((row) => `${row.label}:${row.quantityOut}`)
        .join("|"),
    ].join("::");
  }, [
    transactionDateMode,
  ...
  const outThisMonth = useMemo(() => {
    ...
    return transactions.reduce((sum, row) => {
      ...
      if (yearValue !== selectedYear || monthValue !== currentMonthValue)
        return sum;
  ...
  const animatedTotalItems = useCountUp(
    stats.totalItems,
    !loadingItems,
    itemStatsAnimationKey,
    900,
    0,
  );
  const animatedNoStockItems = useCountUp(
    noStockItemsCount,
    !loadingItems,
    itemStatsAnimationKey,
    900,
  ...
  const animatedLowStock = useCountUp(
    stats.lowStock,
    !loadingItems,
    itemStatsAnimationKey,
    900,
  ...
  const animatedOutThisMonth = useCountUp(
    outThisMonth,
    !loadingTransactions,
    outStatsAnimationKey,
    1000,
    120,
  );
  ...
  const loadItems = async (year: number) => {
    try {
      ...
      const response = await fetch(
        `${getApiUrl()}/api/office-supply/items?${params.toString()}`,
        {
      ...
      await ensureOkResponse(
        response,
        "Unable to load office supply inventory.",
      );
  ...
  const loadReferences = async () => {
    const [departmentResponse, employeeResponse] = await Promise.all([
      ...
      fetch(`${getApiUrl()}/api/employees?status=employed`, {
        headers: { Accept: "application/json" },
      }),
    ]);
    ...
    await ensureOkResponse(
      employeeResponse,
      "Unable to load employed employees.",
    );

    const departmentResult = await departmentResponse.json();
    const employeeResult = await employeeResponse.json();

    const departmentRows = Array.isArray(departmentResult?.data)
      ? departmentResult.data
      : [];
    const employeeRows = Array.isArray(employeeResult?.data)
      ? employeeResult.data
      : [];
  ...
  useEffect(() => {
    setQuantityChangeFx((prev) =>
      prev.active
        ? { ...prev, active: false, amount: 0, zeroHit: false }
        : prev,
    );
  ...
  const handleMovementTypeChange = (nextType: MovementType) => {
    ...
    setTransactionDraft((prev) => ({
      ...
      issued_log:
        nextType === "in"
          ? "RESTOCK"
          : prev.issued_log === "RESTOCK"
            ? ""
            : prev.issued_log,
      requested_by_employee_id:
        nextType === "in" ? (adminSupervisorHrEmployee?.id ?? "") : "",
    }));
  };

  const triggerQuantityChangeFx = (
    mode: QuantityFxMode,
    amount: number,
    zeroHit = false,
  ) => {
  ...
  const executeCreateItem = async () => {
    ...
    try {
      ...
      if (!adminDepartment) {
        const message =
          "No Admin Department was found in departments. Please create it first.";
      ...
      const payload = {
        item_name: normalizeUppercaseInventoryText(
          createItemConfirmDraft.item_name,
        ).trim(),
        category: normalizeUppercaseInventoryText(
          createItemConfirmDraft.category,
        ).trim(),
  ...
  const createItem = async () => {
    ...
    const itemName = normalizeUppercaseInventoryText(
      itemDraft.item_name,
    ).trim();
    ...
    if (!itemName || !category) {
      toast.warning("Missing Item Details", {
        description:
          "Please provide both Item Name and Category before creating the item.",
      });
      return;
    }
  ...
  const saveEditedItem = async (itemId: number) => {
    ...
    const itemName = normalizeUppercaseInventoryText(
      String(itemEditDraft.item_name || ""),
    ).trim();
    const category = normalizeUppercaseInventoryText(
      String(itemEditDraft.category || ""),
    ).trim();

    if (!itemName || !category) {
      toast.warning("Missing Item Details", {
        description:
          "Please provide both Item Name and Category before saving.",
      });
      return;
    }
    ...
    const duplicateExists = items.some(
      (item) =>
        item.id !== itemId &&
        normalizeForDuplicateCheck(item.item_name) ===
          normalizeForDuplicateCheck(itemName),
    );
    if (duplicateExists) {
      toast.warning("Duplicate Item", {
        description: "An inventory item with the same name already exists.",
    ...
    try {
      ...
      const response = await fetch(
        `${getApiUrl()}/api/office-supply/items/${itemId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const removeSelectedItems = async () => {
    ...
    const shouldClearSelectedTxItem =
      selectedTxItemId > 0 && idsToDelete.includes(selectedTxItemId);
    try {
      ...
      const response = await fetch(endpoint, {
        ...
        body: isSingleDelete
          ? undefined
          : JSON.stringify({ item_ids: idsToDelete }),
      });
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Failed to delete inventory item(s).";
    ...
    try {
      ...
      const requesterId =
        movementType === "in"
          ? (adminSupervisorHrEmployee?.id ?? "")
          : transactionDraft.requested_by_employee_id;
      const quantityInValue =
        movementType === "in" ? Number(transactionDraft.quantity_in || 0) : 0;
      const quantityOutValue =
        movementType === "out" ? Number(transactionDraft.quantity_out || 0) : 0;
      if (
        (movementType === "in" && quantityInValue <= 0) ||
        (movementType === "out" && quantityOutValue <= 0)
      ) {
      ...
      if (movementType === "in" && !requesterId) {
        toast.warning("Admin Supervisor/HR Missing", {
          description:
            'No employed employee with position "Admin Supervisor/HR" was found.',
        });
        return;
      }
      ...
      if (
        movementType === "out" &&
        selectedItem &&
        quantityOutValue > Number(selectedItem.current_balance || 0)
      ) {
        toast.warning("Insufficient Stock", {
          description:
            "Quantity Out cannot exceed the selected item current stock balance.",
        });
        return;
      }
      ...
      const payload = {
        ...
        issued_log:
          movementType === "in"
            ? "RESTOCK"
            : transactionDraft.issued_log.trim() || null,
        requested_by_employee_id: requesterId,
      };

      const response = await fetch(
        `${getApiUrl()}/api/office-supply/transactions`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="min-h-screen bg-[#F5F6F8] p-6 md:p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          {Array.from({ length: 4 }).map((_, idx) => (
            <Skeleton
              key={`inventory-stat-skeleton-${idx}`}
      ...
      <main className="w-full max-w-[1700px] mx-auto px-4 md:px-8 py-8 space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          <Card
            ...
            onKeyDown={(event) =>
              triggerCardActionOnKeyDown(event, handleInventoryItemsCardClick)
            }
            role="button"
            tabIndex={0}
          >
          ...
          <Card
            ...
            onKeyDown={(event) =>
              triggerCardActionOnKeyDown(event, handleNoStockCardClick)
            }
            role="button"
            tabIndex={0}
          >
            ...
            {loadingItems ? (
              ...
              <>
                <p className="text-3xl font-black text-rose-700 mt-1 tabular-nums">
                  {animatedNoStockItems}
                </p>
                <p className="text-xs font-semibold text-rose-700/90 mt-1">
                  Items with current balance of 0
                </p>
                <div className="mt-2 h-1.5 rounded-full bg-rose-100 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: statsVisualsAnimated
                        ? `${Math.min(noStockPercentage, 100)}%`
                        : "0%",
                    }}
                  />
                </div>
          ...
          <Card
            ...
            onKeyDown={(event) =>
              triggerCardActionOnKeyDown(event, handleLowStockCardClick)
            }
            role="button"
            tabIndex={0}
          >
              ...
              <>
                <p
                  className={cn(
                    ...
                    stats.lowStock > 0 ? "text-amber-700" : "text-emerald-700",
                  )}
                >
                  {animatedLowStock}
                </p>
                <p className="text-xs font-semibold text-amber-700/90 mt-1">
                  Items with balance from 1 to 10
                </p>
                <div className="mt-2 h-1.5 rounded-full bg-amber-100 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: statsVisualsAnimated
                        ? `${Math.min(lowStockPercentage, 100)}%`
                        : "0%",
                    }}
                  />
                </div>
          ...
          <Card
            ...
            onKeyDown={(event) =>
              triggerCardActionOnKeyDown(event, handleOutThisMonthCardClick)
            }
            role="button"
            tabIndex={0}
          >
            ...
            {loadingTransactions ? (
              ...
              <>
                <p className="text-3xl font-black text-sky-700 mt-1 tabular-nums">
                  {animatedOutThisMonth}
                </p>
        ...
        >
          ...
          >
            <div className="px-5 py-4 border-b border-slate-100 bg-gradient-to-r from-white to-rose-50/40">
              <p className="text-xs font-black uppercase tracking-[0.2em] text-[#A4163A]">
                Item Setup
              </p>
            ...
            <div className="p-5 h-full flex flex-col gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    {isDuplicateItemName ? (
                      <p className="text-[11px] font-semibold text-rose-700 inline-flex items-center gap-1">
                        ...
                        Duplicate item name. Existing item:{" "}
                        {duplicateItemNameMatch?.item_code} -{" "}
                        {duplicateItemNameMatch?.item_name}
                      </p>
                    ) : null}
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      placeholder="e.g. STATIONERY, DESK / FASTENING TOOLS etc."
                      className="rounded-sm h-10 uppercase"
                      disabled={!canEditItemSetup}
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    {categoryOptions.length === 0 ? (
                      <p className="text-xs text-slate-500">
                        No saved categories yet. Create your first category
                        above.
                      </p>
                    ) : (
                      <div className="flex overflow-x-auto gap-2 pb-2 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
                        {categoryOptions.map((category) => {
                          const isSelected =
                            normalizedCategoryPreview.toLowerCase() ===
                            ...
                            <Button
                              ...
                              onClick={() =>
                                setItemDraft((prev) => ({
                                  ...prev,
                                  category:
                                    normalizeUppercaseInventoryText(category),
                                }))
                              }
                              className={cn(
                                "h-7 rounded-full px-3 text-[11px] font-bold",
                                isSelected
                                  ? "border-[#A4163A]/40 bg-rose-50 text-[#A4163A]"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setItemDraft((prev) => ({
                          ...prev,
                          opening_balance: sanitizeIntegerInput(e.target.value),
                        }))
                      }
              ...
              <Button
                ...
                className="w-full bg-[#A4163A] hover:bg-[#8D1332] text-white font-black rounded-lg mt-auto"
              >
                {savingItem ? (
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                ) : (
              ...
              <div className="space-y-4 flex-1">
                <div className="rounded-lg border border-slate-200 bg-white p-4 space-y-4">
                            ...
                            <span className="flex min-w-0 items-center gap-2">
                              ...
                              <Badge
                                className={cn(
                                  ...
                                  getStockBadgeTone(
                                    Number(selectedItem.current_balance || 0),
                                  ),
                                )}
                              >
                        ...
                        <Command>
                          ...
                          <CommandList>
                            ...
                            {groupedItemsByCategory.map((group) => (
                              <CommandGroup
                                key={`item-category-${group.category}`}
                                heading={
                                  <div className="flex items-center justify-between w-full pr-2 pt-1 pb-0.5">
                                    <span className="text-xs font-medium text-slate-500 truncate min-w-0 mr-2">
                                      {group.category}
                                    </span>
                                    <span className="text-[10px] font-bold tracking-widest text-[#A4163A]/80 uppercase shrink-0">
                                      STOCK
                                    </span>
                                  </div>
                                }
                    ...
                    <div
                      className={cn(
                        ...
                        quantityChangeFx.active &&
                          quantityChangeFx.mode === "in"
                          ? "is-in"
                          : "",
                        quantityChangeFx.active &&
                          quantityChangeFx.mode === "out"
                          ? "is-out"
                          : "",
                        quantityChangeFx.active && quantityChangeFx.zeroHit
                          ? "is-zero-hit"
                          : "",
                      )}
                    >
                      <span className="font-semibold truncate relative z-[1]">
                        Selected: {selectedItem.item_code} -{" "}
                        {selectedItem.item_name}
                      </span>
                      <Badge
                        className={cn(
                      ...
                      {quantityChangeFx.active ? (
                        ...
                        >
                          {quantityChangeFx.mode === "in" ? "+" : "-"}
                          {quantityChangeFx.amount}
                        </span>
                      ) : null}
                      <span
                        aria-hidden
                        className="quantity-slot-hollow-flash"
                      />
                    </div>
                  ) : null}
                  <div
                  ...
                  >
                    ...
                    <Input
                      ...
                      value={
                        movementType === "out"
                          ? transactionDraft.quantity_out
                          : transactionDraft.quantity_in
                      }
                      onKeyDown={blockNonIntegerKey}
                      onChange={(e) => {
                    ...
                    {exceedsSelectedStock ? (
                      <p className="text-[11px] font-semibold text-rose-700 inline-flex items-center gap-1">
                        ...
                        {selectedItem?.current_balance ?? 0}).
                      </p>
                    ) : null}
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-slate-200 bg-white p-4 space-y-4">
                  ...
                  >
                    <div
                      className={cn(
                        "min-h-0 space-y-4 pb-1",
                        movementType === "out" ? "" : "pointer-events-none",
                      )}
                    >
                      <div className="space-y-1.5">
                        <Label className="text-xs font-black uppercase tracking-wider text-slate-500">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      className="rounded-sm h-10 bg-slate-50"
                      readOnly
                      disabled
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Textarea
                      value={transactionDraft.issued_log}
                      onChange={(e) =>
                        setTransactionDraft((prev) => ({
                          ...prev,
                          issued_log: e.target.value,
                        }))
                      }
                      placeholder="Purpose for stock movement..."
                      rows={3}
                      className="rounded-sm"
                      disabled={!canEditTransactions || movementType === "in"}
                    />
                  </div>
                </div>
              </div>
        ...
        >
          <div className="px-5 py-4 border-b border-slate-100 bg-slate-50/70 space-y-3">
            <div className="flex flex-col lg:flex-row gap-3 lg:items-center lg:justify-between">
              <div>
                <p className="text-[11px] font-black uppercase tracking-[0.2em] text-[#A4163A]">
                  Inventory Register
                </p>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button
                  ...
                  disabled={
                    savingItemEdit || deletingItems || !canEditItemSetup
                  }
                  className={cn(
                    "h-9 rounded-sm font-black text-xs uppercase tracking-wider",
                    itemEditMode
                      ? "bg-[#A4163A] hover:bg-[#800020] text-white"
                      : "border-[#FFE5EC] text-[#A4163A] hover:bg-rose-50",
                  )}
                >
                  {itemEditMode ? (
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setStockFilter(
                    value as "all" | "no-stock" | "low-stock" | "high-stock",
                  )
                }
              >
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-rose-50/50 border-b border-rose-100">
                    ...
                    <TableHead className="w-[54px] text-center">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          toggleSelectAllItems(checked === true)
                        }
                        disabled={
                  ...
                  <TableHead className="font-black text-[#800020] uppercase tracking-wider text-[11px]">
                    Item Code
                  </TableHead>
                  <TableHead className="font-black text-[#800020] uppercase tracking-wider text-[11px]">
                    Item Name
                  ...
                  <TableHead className="text-right font-black text-[#800020] uppercase tracking-wider text-[11px]">
                    Current
                  </TableHead>
                  <TableHead className="font-black text-[#800020] uppercase tracking-wider text-[11px]">
                    Last Updated
                  ...
                  {itemEditMode ? (
                    <TableHead className="w-[180px] text-right font-black text-[#800020] uppercase tracking-wider text-[11px]">
                      Action
                    </TableHead>
                  ) : null}
                </TableRow>
              ...
              <TableBody>
                {loadingItems ? (
                  ...
                  paginatedFilteredItems.map((item) => {
                    const currentStock = Number(item.current_balance || 0);
                    const isEditingRow =
                      itemEditMode && editingItemId === item.id;
                    const rowToneClass =
                      currentStock === 0
                    ...
                    return (
                      <TableRow
                        key={item.id}
                        className={cn(
                          "border-b animate-in fade-in slide-in-from-bottom-1 duration-300",
                          rowToneClass,
                        )}
                      >
                        {itemEditMode ? (
                        ...
                        <TableCell className="text-right">
                          <Badge
                            className={cn(
                              "rounded-sm border",
                              getStockBadgeTone(currentStock),
                            )}
                          >
                            {item.current_balance}
                          </Badge>
          ...
          <div className="px-5 py-4 border-b border-slate-100 bg-slate-50/70 space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="inline-flex h-9 rounded-md border border-slate-200 bg-white p-1">
                {(
                  [
                    ...
                    { key: "year", label: "Year" },
                  ] as Array<{ key: TransactionDateMode; label: string }>
                ).map((mode) => (
                  <button
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (!Number.isNaN(parsedYear) && parsedYear > 0) {
                      setTransactionYearFilter(
                        Math.min(parsedYear, currentYear),
                      );
                    }
                  }}
              ...
              {transactionDateMode === "month" ? (
                <Select
                  ...
                  onValueChange={setTransactionMonthFilter}
                >
              ...
              {transactionDateMode !== "date" ? (
                <Select
                  ...
                  onValueChange={(value) =>
                    setTransactionYearFilter(
                      Math.min(Number(value), currentYear),
                    )
                  }
                >
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setTransactionTypeFilter(value as "all" | "in" | "out")
                }
              >
              ...
              <Select
                ...
                onValueChange={setTransactionDepartmentFilter}
              >
            ...
            >
              <div className="rounded-lg border border-slate-200 bg-white p-4 space-y-3">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-600">
                    Top Items by Quantity Out
                  </p>
                  <Select
                    value={visualizationTopLimit}
                    onValueChange={(value) =>
                      setVisualizationTopLimit(value as VisualizationTopLimit)
                    }
                  >
                ...
                {topItemsByQuantityOut.length === 0 ? (
                  ...
                  <>
                      ...
                      {topItemsByQuantityOut.map((row, index) => (
                        <div
                          ...
                          className={cn(
                            ...
                            hoveredTopItemKey === row.key
                              ? "bg-rose-50/70"
                              : "hover:bg-slate-50",
                          )}
                          onMouseEnter={() => setHoveredTopItemKey(row.key)}
                          onMouseLeave={() =>
                            setHoveredTopItemKey((prev) =>
                        ...
                        >
                          ...
                          <div className="h-2 rounded-full bg-slate-100 overflow-hidden">
                            <div
                              ...
                              style={{
                                width: visualizationAnimated
                                  ? `${(row.quantityOut / topItemsByQuantityOutMax) * 100}%`
                                  : "0%",
                    ...
                    >
                      {hoveredTopItemKey ? (
                        <div
                          key={`top-item-hover-content-${hoveredTopItemKey}`}
                        ...
                        >
                          <p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-600 animate-in fade-in-0 slide-in-from-top-1 duration-200">
                            {topItemsByQuantityOut.find(
                              (row) => row.key === hoveredTopItemKey,
                            )?.label || "Item"}{" "}
                            by Department
                          </p>
                          <div className="mt-2 space-y-1.5">
                            {(
                              topItemDepartmentBreakdown[hoveredTopItemKey] ??
                              []
                            ).length === 0 ? (
                              <p className="text-xs font-semibold text-slate-500 animate-in fade-in-0 slide-in-from-bottom-1 duration-200">
                              ...
                              (
                                ...
                                <div
                                  ...
                                  style={{
                                    animationDelay: `${Math.min(entryIndex * 45, 220)}ms`,
                                  }}
                                >
                        ...
                        <p className="text-xs font-semibold text-slate-500 animate-in fade-in-0 slide-in-from-bottom-1 duration-200">
                          Hover an item bar to see which departments contributed
                          to its quantity out.
                        </p>
                      )}
                    </div>
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-4 space-y-3">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-600">
                    Department Usage (Quantity Out)
                  </p>
                </div>
                {departmentOutData.length === 0 ? (
                  <p className="text-sm font-semibold text-slate-500">
                    No outgoing department movement yet.
                  </p>
                ) : (
                  <>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div
                              ...
                              onMouseEnter={() =>
                                setHoveredDepartmentLabel(row.label)
                              }
                              onMouseLeave={() =>
                                setHoveredDepartmentLabel((prev) =>
                                  prev === row.label ? null : prev,
                                )
                              }
                            >
                              <div
                    ...
                    >
                      {hoveredDepartmentLabel ? (
                        <div
                          key={`department-hover-content-${hoveredDepartmentLabel}`}
                          className="animate-in fade-in-0 zoom-in-95 duration-200"
                        >
                          <p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-600 animate-in fade-in-0 slide-in-from-top-1 duration-200">
                          ...
                          <div className="mt-2 space-y-1.5">
                            {(
                              departmentItemBreakdown[hoveredDepartmentLabel] ??
                              []
                            ).length === 0 ? (
                              <p className="text-xs font-semibold text-slate-500 animate-in fade-in-0 slide-in-from-bottom-1 duration-200">
                        ...
                        <p className="text-xs font-semibold text-slate-500 animate-in fade-in-0 slide-in-from-bottom-1 duration-200">
                          Hover a department out bar to see which items drove
                          its quantity out.
                        </p>
                      )}
                    </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50 border-b border-slate-100">
                  <TableHead className="font-black uppercase tracking-wider text-[11px] text-slate-600">
                    Item Code
                  </TableHead>
                  <TableHead className="font-black uppercase tracking-wider text-[11px] text-slate-600">
                    Item Name
                  ...
                  <TableHead className="text-right font-black uppercase tracking-wider text-[11px] text-slate-600">
                    Beginning Balance
                  </TableHead>
                  <TableHead className="text-right font-black uppercase tracking-wider text-[11px] text-slate-600">
                    Quantity In
              ...
              <TableBody>
                {loadingTransactions ? (
                  Array.from({ length: 6 }).map((_, idx) => (
                    <TableRow
                      key={`transaction-loading-row-${idx}`}
                      className="border-b border-slate-100"
                    >
                      <TableCell>
                        <Skeleton className="h-5 w-16" />
                  ...
                  paginatedTransactions.map((row) => (
                    <TableRow
                      key={row.id}
                      className="border-b border-slate-100 hover:bg-slate-50/60 animate-in fade-in slide-in-from-bottom-1 duration-300"
                    >
                      <TableCell className="font-semibold text-slate-700">
                        {row.item_code || "-"}
                      </TableCell>
                      ...
                      <TableCell>
                        {getTransactionDepartmentLabel(row)}
                      </TableCell>
                      <TableCell className="text-right">
                      ...
                      <TableCell className="text-right text-rose-700 font-semibold">
                        {row.quantity_out}
                      </TableCell>
          ...
          {noStockItemsCount > 0 ? (
            ...
            >
              <p className="text-[12px] font-bold text-rose-700 inline-flex items-center gap-2">
                <AlertTriangle className="h-4 w-4" />
                {noStockItemsCount} item
                {noStockItemsCount > 1 ? "s have" : " has"} 0 stock. Please
                restock immediately.
              </p>
            </Card>
          ) : null}
          ...
          {stats.lowStock > 0 ? (
            ...
            >
              <p className="text-[12px] font-bold text-amber-700 inline-flex items-center gap-2">
                <AlertTriangle className="h-4 w-4" />
                {stats.lowStock} item{stats.lowStock > 1 ? "s are" : " is"} in
                low stock (1 to 10). Consider restocking soon.
              </p>
            </Card>
          ) : null}
        ...
        <footer className="sticky bottom-0 w-full bg-white border-t border-slate-200 py-3 px-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-40 flex items-center overflow-hidden">
          <div className="mx-auto w-full max-w-[1700px] flex items-center gap-4 text-xs font-bold whitespace-nowrap">
            <span className="shrink-0 text-slate-500 pr-4 border-r border-slate-200">
              INVENTORY ALERTS
            </span>
            {noStockItemsCount > 0 || stats.lowStock > 0 ? (
              <div
                ...
                <div className="animate-marquee inline-flex gap-8 whitespace-nowrap items-center min-w-full">
                  {noStockItemsCount > 0 && (
                    <span className="text-rose-700 inline-flex items-center gap-2">
                      ...
                      {noStockItemsCount} items have 0 stock. Please restock
                      immediately.
                    </span>
                  )}
                  {stats.lowStock > 0 && (
                    <span className="text-amber-700 inline-flex items-center gap-2">
                      <AlertTriangle className="h-4 w-4 shrink-0" />
                      {stats.lowStock} items are in low stock (1 to 10).
              ...
              <span className="text-emerald-700 inline-flex items-center gap-2">
                <PackageCheck className="h-4 w-4" /> All tracked inventory items
                are above low-stock threshold.
              </span>
            )}
          </div>
        </footer>
                  ...
                  <div className="space-y-1">
                    <AlertDialogTitle className="text-xl font-black text-rose-800">
                      {itemIdsToDelete.length > 1
                        ? `Delete ${itemIdsToDelete.length} inventory items?`
                ...
                <div className="space-y-1">
                  ...
                  >
                    {isTransactionFxSuccess
                      ...
                      : isTransactionFxError
                        ? transactionFxError ||
                          "Unable to save this stock movement. Please retry."
            ...
            <div className="px-6 pt-6 pb-2">
              ...
              <div className="mt-4 rounded-xl border border-rose-100 bg-rose-50/50 p-4 space-y-2 text-left shadow-[inset_0_1px_0_0_rgba(255,255,255,0.6)]">
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className="text-sm font-bold text-[#A4163A]">
                    {createItemConfirmDraft?.opening_balance ?? 0}
                  </span>
                </div>
              </div>
              <div
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      {isCreateItemSuccess
                        ...
                        : isCreateItemError
                          ? createItemFxError ||
                            "The request was rejected. Review details and retry."
        ...
        <style jsx>{`
          ...
          .create-item-fx-icon.is-storing .create-item-fx-item {
            animation: create-item-drop 1.1s cubic-bezier(0.23, 0.7, 0.3, 1)
              infinite;
          }
          ...
          .create-item-fx-icon.is-success .create-item-fx-shell {
            border-color: #109867;
            animation: create-item-shell-bulge 450ms
          ...
          .create-item-fx-icon.is-error {
            ...
            animation: create-item-error-shake 440ms
          ...
          .transaction-fx-icon.is-error {
            border-color: #ffc6cc;
            background: linear-gradient(180deg, #fff5f6 0%, #ffe9eb 100%);
            animation: transaction-fx-error-shake 450ms
              cubic-bezier(0.36, 0.07, 0.19, 0.97);
          }
