    ...
    const fetchYears = async () => {
      try {
        const [tardinessRes, leavesRes] = await Promise.all([
          fetch(`${getApiUrl()}/api/admin/attendance/tardiness/years`),
        ...
        <div className="w-full px-4 md:px-8 py-6">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <Button
                onClick={() =>
                  router.push("/admin/attendance/warning-letter/edit_forms")
                }
                disabled={isViewOnly}
                variant="outline"
                      ...
                      >
                        ...
                        <TableCell className="py-5 px-8 text-center">
                          <Button
                            ...
                            onClick={() => {
                              router.push(
                                `/admin/attendance/warning-letter/forms-letter?employeeId=${entry.employee_id}&type=late&month=${selectedMonth}&year=${selectedYear}&cutoff=${(entry as any).cutoff}&warningLevel=${entry.warning_level || 1}`,
                              );
                            }}
                      ...
                      >
                        ...
                        <TableCell className="py-5 px-8 text-center">
                          <Button
                            ...
                            onClick={() => {
                              router.push(
                                `/admin/attendance/warning-letter/forms-letter?employeeId=${entry.employee_id}&type=leave&month=${selectedMonth}&year=${selectedYear}&cutoff=${(entry as any).cutoff}&warningLevel=${entry.warning_level || 1}`,
                              );
                            }}
                            className="text-[#800020] hover:text-[#800020] hover:bg-rose-50/50 rounded-xl font-bold gap-2 text-xs h-10 px-6 border border-transparent hover:border-rose-100 transition-all shadow-none hover:shadow-sm cursor-pointer"
                          >
                  ...
                  >
                    ...
                    <div className="pt-2 flex justify-end">
                      <Button
                        ...
                        onClick={() => {
                          router.push(
                            `/admin/attendance/warning-letter/forms-letter?employeeId=${letter.employee_id}&type=${normalizeWarningType(letter.type)}&month=${letter.month}&year=${letter.year}&cutoff=${letter.cutoff}&warningLevel=${letter.warning_level || 1}&mode=review&letterId=${letter.id}`,
                          );
                        }}
