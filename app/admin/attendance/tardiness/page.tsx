...
let DYNAMIC_SHIFT_SCHEDULES: Record<string, ShiftInfo> = {
...
function getShiftSchedule(department?: string): ShiftInfo {
  const fallbackKey =
    Object.keys(DYNAMIC_SHIFT_SCHEDULES).find((k) => k.includes("ABIC")) ||
    "ABIC";
  const fallback =
    DYNAMIC_SHIFT_SCHEDULES[fallbackKey] || DEFAULT_SHIFT_SCHEDULES["ABIC"];
  ...
  if (DYNAMIC_SHIFT_SCHEDULES[normalized])
    return DYNAMIC_SHIFT_SCHEDULES[normalized];

  // 2. Try mapping from department name to office
  const mappedOffice = DEPT_TO_OFFICE_MAP[normalized];
  if (mappedOffice) {
    if (DYNAMIC_SHIFT_SCHEDULES[mappedOffice])
      return DYNAMIC_SHIFT_SCHEDULES[mappedOffice];

    // Fuzzy match mapped office against existing schedules
    const partialMapped = Object.keys(DYNAMIC_SHIFT_SCHEDULES).find(
      (off) => mappedOffice.includes(off) || off.includes(mappedOffice),
    );
    if (partialMapped) return DYNAMIC_SHIFT_SCHEDULES[partialMapped];
  }

  // 3. Try fuzzy/partial match on department name itself
  const partialMatch = Object.keys(DYNAMIC_SHIFT_SCHEDULES).find(
    (off) => normalized.includes(off) || off.includes(normalized),
  );
...
function getEffectiveSchedule(
  ...
  const leave = leaves.find(
    (l) =>
      (String(l.employee_id) === String(employeeId) ||
        (employeeName &&
          l.employee_name &&
          l.employee_name.toLowerCase() === employeeName.toLowerCase())) &&
      l.approved_by !== "Pending" &&
      l.approved_by !== "Declined" && // Approved
      normalizeDate(l.start_date) <= normalizedDate &&
      normalizeDate(l.leave_end_date) >= normalizedDate,
  );

  if (!leave) return schedule;

  const leaveCategory = (leave.category || "")
    .toLowerCase()
    .replace(/[\s_-]+/g, "");
  ...
  if (
    leaveCategory.includes("half") &&
    leave.shift &&
    schedule.allShifts &&
    schedule.allShifts.length > 1
  ) {
  ...
  try {
    ...
    if (deptData.success && officeData.success) {
      ...
      departments.forEach((dept: any) => {
        ...
        if (off) {
          DEPT_TO_OFFICE_MAP[dept.name.toUpperCase().trim()] = off.name
            .toUpperCase()
            .trim();
        }
      });
...
function calculateMinutesLate(
  ...
  const schedule =
    employeeId && date
      ? getEffectiveSchedule(employeeId, employeeName, date, department, leaves)
      : getShiftSchedule(department);
    ...
    const processedGroup = groupEntries.map((entry) => {
      ...
      if (
        ...
        exceedsGracePeriod(
          ...
          department,
          entry.employee_id,
          entry.employee_name || entry.employeeName,
          entry.date,
          leaves,
        )
      ) {
...
function exportToExcel(
  ...
  const totalEmployeesWithLates = summaryArray.filter(
    (e) => e.occurrences > 0,
  ).length;
  const totalLateMinutes = summaryArray.reduce(
    (sum, e) => sum + e.totalMinutes,
    0,
  );
  const totalOccurrences = summaryArray.reduce(
  ...
  AOA.push(["Notes:", "", "", ""]);
  AOA.push([
    "* Minutes are counted from each employee's office start time",
    "",
    "",
    "",
  ]);
  AOA.push([
    "* Shift times: ABIC=8:00 AM, INFINITECH=9:00 AM, G-LIMIT=10:00 AM",
    "",
    "",
    "",
  ]);
  AOA.push([
    "* Total Lates count is based on arrivals after respective grace periods (5 minutes)",
    "",
    "",
    "",
  ]);
  ...
  // Column widths
  ws["!cols"] = [{ wch: 34 }, { wch: 22 }, { wch: 30 }, { wch: 14 }];
  ...
  S(summaryHeaderRow, 0, {
    ...
    border: {
      top: thin(INDIGO),
      bottom: thin(BORDER_CLR),
      left: thin(INDIGO),
      right: thin(BORDER_CLR),
    },
  });
  ...
  summaryData.forEach((row, idx) => {
    ...
    S(r, 0, {
      ...
      fill: {
        patternType: "solid",
        fgColor: { rgb: isEven ? WHITE : SLATE_ALT },
      },
      alignment: { horizontal: "left", vertical: "center" },
      border: border(),
    });
  ...
  XLSX.writeFile(
    wb,
    `Tardiness_Summary_${cutoffTitle.replace(/[^a-zA-Z0-9]/g, "_")}_${month}_${year}.xlsx`,
  );
}
...
function isFuzzyMatch(term: string, target: string): boolean {
  ...
  for (let k = 0; k < target.length; k++) {
    for (
      let len = term.length - maxTypos;
      len <= term.length + maxTypos;
      len++
    ) {
      if (len < 1 || k + len > target.length) continue;
      const sub = target.substring(k, k + len);

      const matrix = Array(sub.length + 1)
        .fill(null)
        .map(() => Array(term.length + 1).fill(0));
...
function SummarySheet({
  ...
  const summaryMap = new Map<
    string | number,
    { totalMinutes: number; occurrences: number; name: string }
  >();
  ...
  const totalLateMinutes = summaryArray.reduce(
    (sum, emp) => sum + emp.totalMinutes,
    0,
  );
  const totalOccurrences = summaryArray.reduce(
    (sum, emp) => sum + emp.occurrences,
    0,
  );
  const totalWarnings = summaryArray.reduce(
    (sum, emp) => sum + emp.warnings,
    0,
  );
      ...
      <SheetContent className="w-full sm:max-w-2xl bg-white border-l-4 border-[#A4163A] text-slate-900 overflow-y-auto p-0">
        ...
        <div className="p-6 space-y-6">
              ...
              <div className="overflow-auto max-h-[60vh]">
                <table className="w-full text-sm">
                  <thead className="bg-[#FFE5EC]/30 sticky top-0 border-b border-[#FFE5EC]">
                    <tr>
                      <th className="px-4 py-3 text-left font-bold text-[#800020] text-xs uppercase tracking-wider">
                        EMPLOYEE NAME
                      </th>
                      <th className="px-4 py-3 text-left font-bold text-[#800020] text-xs uppercase tracking-wider">
                        TOTAL LATES (MINS)
                  ...
                  <tbody className="divide-y divide-rose-50">
                    {summaryPagination.paginatedItems.length > 0 ? (
                      summaryPagination.paginatedItems.map((emp, idx) => (
                        <tr
                          key={idx}
                          className="hover:bg-[#FFE5EC] transition-colors duration-200"
                        >
          ...
          <div className="text-xs text-stone-500 space-y-1">
            <p>
              * Minutes are counted from assigned shift start time (no grace
              period in minutes display)
            </p>
            <p>
              * Occurrences are only counted for arrivals after the 5-minute
              grace period
            </p>
...
export default function AttendanceDashboard() {
    ...
    const fetchYears = async () => {
      try {
        const res = await fetch(
          `${getApiUrl()}/api/admin/attendance/tardiness/years`,
        );
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [empRes, hierRes, deptRes, leavesRes, offRes] = await Promise.all(
          [
            fetch(
              "/api/admin/employees?status=employed,rehired,rehired_employee",
            ),
        ...
        if (empData.success) {
          ...
          const resolvedEmployees = empData.data.map((e: any) => {
            ...
            if (!deptName && e.position && hierarchies.length > 0) {
              const hier = hierarchies.find(
                (h: any) => h.name.toLowerCase() === e.position?.toLowerCase(),
              );
              if (hier && hier.department_id) {
                const d = departments.find(
                  (d: any) => String(d.id) === String(hier.department_id),
                );
          ...
          const entRes = await fetch(
            `${getApiUrl()}/api/admin/attendance/tardiness?month=${selectedMonth}&year=${selectedYear}`,
          );
          const entData = await entRes.json();

          if (entData.success) {
            const mappedEntries = entData.data.map((e: any) => {
              const empInfo = resolvedEmployees.find(
              ...
              return {
                ...
                minutesLate: calculateMinutesLate(
                  e.actual_in,
                  empInfo?.department,
                  e.employee_id,
                  e.employee_name,
            ...
            const sorted = mappedEntries.sort((a: any, b: any) => {
              ...
              if (dateA !== dateB) return dateA - dateB;
              return (
                parseTimeToMinutes(a.actual_in) -
                parseTimeToMinutes(b.actual_in)
              );
            });
  ...
  const firstCutoffEntries = filteredByPeriod.filter(
    (e) => e.cutoffPeriod === "cutoff1",
  );
  const secondCutoffEntries = filteredByPeriod.filter(
    (e) => e.cutoffPeriod === "cutoff2",
  );

  // Summary visibility state - controlled by Summary buttons
  const [summarySheetOpen, setSummarySheetOpen] = useState(false);
  const [activeCutoffSummary, setActiveCutoffSummary] = useState<{
    title: string;
    entries: LateEntry[];
  } | null>(null);
  ...
  const handleUndoYear = async (year: number) => {
    try {
      const res = await fetch(
        `/api/admin/attendance/tardiness/years?year=${year}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleDeleteEntry = async (id: string | number) => {
    try {
      const res = await fetch(
        `${getApiUrl()}/api/admin/attendance/tardiness/${id}`,
        {
          method: "DELETE",
        },
      );
  ...
  const addNewYear = async () => {
    ...
    try {
      const res = await fetch("/api/admin/attendance/tardiness/years", {
        method: "POST",
  ...
  const debounceTimers = useRef<
    Record<string | number, ReturnType<typeof setTimeout>>
  >({});
  ...
  const updateEntryTime = (id: string | number, newTime: string) => {
    ...
    const employee = employees.find((e) => {
      ...
      return (
        String(e.id) === entryEmpId || e.name === entryToUpdate?.employee_name
      );
    });

    const department =
      employee?.department ||
      entryToUpdate?.department ||
      entryToUpdate?.office;
    // Validation: Check for approved leave(s) that cover this time
    ...
    const dayLeaves = leaves.filter(
      (l) =>
        ...
        l.approved_by !== "Pending" &&
        l.approved_by !== "Declined" && // Approved
        normalizeDate(l.start_date) <= dateStr &&
        normalizeDate(l.leave_end_date) >= dateStr,
    );

    for (const leave of dayLeaves) {
      if (leave.category === "whole-day") {
        toast.error(
          `${entryToUpdate?.employee_name} has an approved whole-day leave on this day. Tardiness cannot be recorded.`,
        );
        return;
      }

      if (leave.category === "half-day" && leave.shift) {
        // Robust split for shift range (handles both hyphen and en-dash)
        const parts = leave.shift.split(/\s*[–-]\s*/);
        if (parts.length === 2) {
          ...
          if (entryMins >= startMins && entryMins <= endMins) {
            toast.error(
              `${entryToUpdate?.employee_name} has an approved half-day leave during this time (${leave.shift}). Tardiness cannot be recorded within the leave period.`,
            );
            return;
          }
        }
    ...
    const minutesLate = calculateMinutesLate(
      newTime,
      department,
      entryToUpdate?.employee_id,
      entryToUpdate?.employee_name,
      entryToUpdate?.date,
      leaves,
    );
  ...
  const updateFirstCutoffTime = (id: string | number, newTime: string) =>
    updateEntryTime(id, newTime);
  const updateSecondCutoffTime = (id: string | number, newTime: string) =>
    updateEntryTime(id, newTime);
  ...
  const handleSaveClick = () => {
    ...
    if (duplicateEntry) {
      toast.error(
        `${newEntryEmployee} already has a late entry recorded for today (${dateStr}).`,
      );
      return;
    }

    // Validation 3: Check for approved leave(s)
    const dayLeaves = leaves.filter(
      (l) =>
        String(l.employee_id) === String(selectedEmployee.id) &&
        l.approved_by !== "Pending" &&
        l.approved_by !== "Declined" &&
        normalizeDate(l.start_date) <= dateStr &&
        normalizeDate(l.leave_end_date) >= dateStr,
    );
    ...
    for (const leave of dayLeaves) {
      if (leave.category === "whole-day") {
        toast.error(
          `${newEntryEmployee} has an approved whole-day leave for today. Tardiness cannot be recorded.`,
        );
        return;
      }
      ...
      if (leave.category === "half-day" && leave.shift) {
        ...
        if (parts.length === 2) {
          ...
          if (entryMins >= startMins && entryMins <= endMins) {
            toast.error(
              `${newEntryEmployee} has an approved half-day leave for this period (${leave.shift}). Tardiness cannot be recorded within the leave period.`,
            );
            return;
          }
        }
  ...
  const executeSaveEntry = async () => {
    ...
    const autoCutoff: "cutoff1" | "cutoff2" =
      dayOfMonth <= 15 ? "cutoff1" : "cutoff2";
    ...
    try {
      const response = await fetch(
        `${getApiUrl()}/api/admin/attendance/tardiness`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        if (entData.success) {
          const mapped = entData.data.map((e: any) => {
            const empInfo = employees.find(
              (emp) => String(emp.id) === String(e.employee_id),
            );
            return {
              ...e,
              date: formatDate(e.date),
          ...
          const sorted = mapped.sort((a: any, b: any) => {
            ...
            if (dateA !== dateB) return dateA - dateB;
            return (
              parseTimeToMinutes(a.actual_in || a.actualIn) -
              parseTimeToMinutes(b.actual_in || b.actualIn)
            );
          });
      ...
      <div className="bg-white p-3 md:p-6 rounded-lg shadow-lg border-b-2 md:border-2 border-[#FFE5EC] space-y-6">
        ...
        >
          ...
          <div className="col-span-full text-right mt-2 text-md text-[#A0153E] font-medium italic">
            * Table shows minutes from assigned shift start time • Summary
            counts arrivals after 5th minute grace period
          </div>
        </div>
      </div>
                      ...
                      <div className="flex items-center justify-center gap-1.5 text-rose-600 font-black text-xs">
                        ...
                        <span>
                          ...
                          {entry.warningLevel === 1
                            ? "st"
                            : entry.warningLevel === 2
                              ? "nd"
                              : entry.warningLevel === 3
                                ? "rd"
                                : "th"}
                        </span>
                      </div>
