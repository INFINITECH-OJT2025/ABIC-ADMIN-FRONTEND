...
const statusBadgeColors = {
  ...
  termination_pending:
    "bg-rose-50 text-rose-700 border-rose-200 shadow-none uppercase tracking-wider",
  resignation_pending:
    "bg-rose-50 text-rose-700 border-rose-200 shadow-none uppercase tracking-wider",
};
...
export default function MasterfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "all" | "employed" | "terminated" | "pending"
  >("all");
  ...
  const [rehireBatchProgress, setRehireBatchProgress] = useState<
  ...
  const fetchPositionsAndDepartments = async () => {
    try {
      const [hierRes, deptRes] = await Promise.all([
      ...
      if (hierRes.ok) {
        ...
        const rows: Hierarchy[] = Array.isArray(data?.data)
          ? data.data
          : Array.isArray(data)
            ? data
            : [];
      ...
      console.error("Failed to fetch positions/departments:", err);
    }
  };
      ...
      if (!isEditingCurrent) {
        ...
        if (selectedEmployee.region) {
          const region = regions.find(
            (r) => r.name === selectedEmployee.region,
          );
          if (region) {
  ...
  const handleSaveSection = async (
    ...
    try {
      ...
      if (data.success) {
        ...
        else if (section === "employment") setIsEditingEmployment(false);
        else if (section === "personal") setIsEditingPersonal(false);
          ...
          empData.data.map(async (emp: Employee) => {
            ...
            const termination = terminationsList
              .filter((t: any) => {
                ...
                const terminationEmail = normalizeName(
                  t?.employee?.email ?? "",
                );
                const emailMatch =
                  Boolean(empEmail) && terminationEmail === empEmail;
            ...
            if (checklist) {
              ...
              const rehireStartedAt = new Date(
                enhancedEmp?.rehire_started_at ?? 0,
              ).getTime();
              const rehireCycleFallbackStart = new Date(
                enhancedEmp?.updated_at ?? 0,
              ).getTime();
              const rehireCycleStartAt =
                Number.isFinite(rehireStartedAt) && rehireStartedAt > 0
                  ? rehireStartedAt
  ...
  const employedList = filterEmployees(
    employees.filter(
      (e) => e.status === "employed" || e.status === "rehired_employee",
    ),
  );
                ...
                <div className="flex flex-wrap items-center gap-4 lg:gap-8">
                  ...
                  <div className="flex items-center bg-white/10 p-1 rounded-lg backdrop-blur-md border border-white/10">
                    <button
                      ...
                      className={`cursor-pointer px-4 py-1.5 rounded-md text-xs font-bold transition-all duration-200 uppercase tracking-wider ${
                        activeTab === "all"
            ...
            {fetchError ? (
              <div className="flex flex-col items-center justify-center py-24 text-center animate-in fade-in zoom-in-95 duration-500">
                <div className="w-20 h-20 bg-rose-50 rounded-full flex items-center justify-center mb-6 group">
                  <Badge
                    variant="outline"
                    className="h-12 w-12 border-rose-200 bg-white shadow-sm flex items-center justify-center rounded-lg group-hover:scale-110 transition-transform duration-300"
                  >
                ...
                <p className="text-slate-500 max-w-md mx-auto mb-8">
                  {fetchError} Please ensure the backend server is running and
                  try again.
                </p>
                <Button
                      ...
                      {pendingList.map((employee) => {
                        ...
                        const checklistTasksComplete =
                          ...
                          (Number(employee.onboarding_tasks?.total ?? 0) > 0 &&
                            Number(employee.onboarding_tasks?.done ?? 0) ===
                              Number(employee.onboarding_tasks?.total ?? 0));

                        // Use default logic if NOT termination pending
                        const { isComplete, status, batchId } =
                                ...
                                {isRehirePending && !isFullyComplete && (
                                  <Button
                                    ...
                                    onClick={() =>
                                      router.push(
                                        !employee.onboarding_tasks?.isComplete
                                          ? `/admin/employee/onboard?id=${employee.id}&view=checklist&rehire=1&batch=${displayBatchId}`
                                          : `/admin/employee/onboard?id=${employee.id}&view=update-info&rehire=1&batch=${displayBatchId}`,
                                      )
                                    }
                                  ...
                                  (!isComplete ||
                                    ...
                                    <Button
                                      ...
                                      onClick={() =>
                                        router.push(
                                          employee.onboarding_tasks?.isComplete
                                            ? `/admin/employee/onboard?id=${employee.id}&batch=${checkCompleteness(employee as any).batchId}`
                                            : `/admin/employee/onboard?id=${employee.id}&view=checklist&batch=${checkCompleteness(employee as any).batchId}`,
                                        )
                                      }
                                      className={`h-8 px-3 text-[11px] font-bold border rounded-lg transition-all whitespace-nowrap ${
                                        employee.onboarding_tasks?.isComplete
                            ...
                            <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-slate-100">
                              {isFullyComplete ? (
                                ...
                                <>
                                  <Badge
                                    variant="outline"
                                    className={`text-[10px] font-bold shadow-none uppercase tracking-wider rounded-md whitespace-nowrap ${isRehirePending ? "text-blue-700 bg-blue-50 border-blue-200" : "text-orange-600 bg-orange-50 border-orange-200"}`}
                                  >
                  ...
                  <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                    <div>
                      ...
                      {isViewOnly && (
                        <p className="text-yellow-200 text-xs md:text-sm font-semibold mt-2 flex items-center gap-1">
                          <Eye className="w-4 h-4" />
                          VIEW ONLY MODE - Editing and modifications are
                          disabled
                        </p>
                      )}
                    </div>
                ...
                <div className="bg-white p-3 md:p-6 rounded-lg shadow-lg border-b-2 md:border-2 border-[#FFE5EC]">
                  {isDetailLoading ? (
                    <div className="flex items-center justify-center p-12">
                      <svg
                        className="w-8 h-8 animate-spin text-[#A0153E]"
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="bg-white border-2 border-[#FFE5EC] shadow-md overflow-hidden rounded-lg">
                        <div className="bg-gradient-to-r from-[#4A081A]/10 to-transparent pb-3 border-b-2 border-[#630C22] p-4">
                          <div className="flex justify-between items-center">
                            <h2 className="text-xl text-[#4A081A] font-bold">
                              Employee Profile
                            </h2>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                                  <div className="space-y-1.5">
                                    ...
                                    <select
                                      ...
                                      className="flex h-9 w-full rounded-md border border-slate-200 px-3 py-1 text-sm font-medium"
                                    >
                                      <option value="">
                                        Select Position...
                                      </option>
                            ...
                            <div className="p-6">
                              {isEditingPersonal ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
                                  ...
                                  <DetailItem
                                    label="Birthplace"
                                    value={selectedEmployee.birthplace}
                                    required
                                  />
                                  <DetailItem
                                    label="Gender"
                                  ...
                                  <DetailItem
                                    label="Civil Status"
                                    value={selectedEmployee.civil_status}
                                    required
                                  />
                                </div>
                              )}
                                    ...
                                    <DetailItem
                                      ...
                                      value={
                                        selectedEmployee.mobile_number
                                          ? `+63 ${selectedEmployee.mobile_number}`
                                          : ""
                                      }
                                      required
                                    />
                              ...
                              <div className="p-6">
                                {isEditingCurrent ? (
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                                    ...
                                    <div className="space-y-1.5">
                                      ...
                                      <select
                                        ...
                                        value={
                                          editFormData.city_municipality || ""
                                        }
                                        onChange={handleEditChange}
                                        disabled={
                                  ...
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-6">
                                    <DetailItem
                                      label="House No."
                                      value={selectedEmployee.house_number}
                                    />
                                    <DetailItem
                                      label="Street"
                                    ...
                                    <DetailItem
                                      label="Village"
                                      value={selectedEmployee.village}
                                    />
                                    <DetailItem
                                      label="Subdivision"
                                      value={selectedEmployee.subdivision}
                                    />
                                    <DetailItem
                                      label="Barangay"
                                      value={selectedEmployee.barangay}
                                      required
                                    />
                                    <DetailItem
                                      label="City / Municipality"
                                      value={selectedEmployee.city_municipality}
                                      required
                                    />
                                    <DetailItem
                                      label="Province"
                                      value={selectedEmployee.province}
                                      required
                                    />
                                    <DetailItem
                                      label="Region"
                              ...
                              <div className="p-6">
                                {isEditingPermanent ? (
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                                    <div className="space-y-1.5">
                                      ...
                                      <Input
                                        ...
                                        value={
                                          editFormData.perm_house_number || ""
                                        }
                                        onChange={handleEditChange}
                                        className="h-9"
                                      />
                                    ...
                                    <div className="space-y-1.5">
                                      ...
                                      <Input
                                        name="perm_street"
                                        value={editFormData.perm_street || ""}
                                        onChange={handleEditChange}
                                        className="h-9"
                                      />
                                    </div>
                                    <div className="space-y-1.5">
                                      ...
                                      <Input
                                        name="perm_village"
                                        value={editFormData.perm_village || ""}
                                        onChange={handleEditChange}
                                        className="h-9"
                                      />
                                    </div>
                                    ...
                                    <div className="space-y-1.5">
                                      ...
                                      <select
                                        name="perm_region"
                                        value={editFormData.perm_region || ""}
                                        onChange={handleEditChange}
                                        className="flex h-9 w-full rounded-md border border-slate-200 px-3 py-1 text-sm font-medium"
                                      >
                                        <option value="">
                                    ...
                                    <div className="space-y-1.5">
                                      ...
                                      <select
                                        name="perm_province"
                                        value={editFormData.perm_province || ""}
                                        onChange={handleEditChange}
                                        disabled={
                                          !editFormData.perm_region ||
                                          loadingProvinces
                                        }
                                        className="flex h-9 w-full rounded-md border border-slate-200 px-3 py-1 text-sm font-medium"
                                      >
                                    ...
                                    <div className="space-y-1.5">
                                      ...
                                      <select
                                        ...
                                        value={
                                          editFormData.perm_city_municipality ||
                                          ""
                                        }
                                        onChange={handleEditChange}
                                        disabled={
                                          !editFormData.perm_province ||
                                          loadingCities
                                        }
                                        className="flex h-9 w-full rounded-md border border-slate-200 px-3 py-1 text-sm font-medium"
                                      >
                                    ...
                                    <div className="space-y-1.5">
                                      ...
                                      <select
                                        name="perm_barangay"
                                        value={editFormData.perm_barangay || ""}
                                        onChange={handleEditChange}
                                        disabled={
                                          !editFormData.perm_city_municipality ||
                                          loadingBarangays
                                    ...
                                    <div className="space-y-1.5">
                                      ...
                                      <Input
                                        name="perm_zip_code"
                                        value={editFormData.perm_zip_code || ""}
                                        onChange={handleEditChange}
                                        className="h-9"
                                      />
                                    </div>
                                  ...
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-6">
                                    <DetailItem
                                      label="House No."
                                      value={selectedEmployee.perm_house_number}
                                    />
                                    <DetailItem
                                      label="Street"
                                      value={selectedEmployee.perm_street}
                                      required
                                    />
                                    <DetailItem
                                      label="Village"
                                      value={selectedEmployee.perm_village}
                                    />
                                    <DetailItem
                                      label="Subdivision"
                                      value={selectedEmployee.perm_subdivision}
                                    />
                                    <DetailItem
                                      label="Barangay"
                                      value={selectedEmployee.perm_barangay}
                                      required
                                    />
                                    <DetailItem
                                      label="City / Municipality"
                                      value={
                                        selectedEmployee.perm_city_municipality
                                      }
                                      required
                                    />
                                    <DetailItem
                                      label="Province"
                                      value={selectedEmployee.perm_province}
                                      required
                                    />
                                    <DetailItem
                                      label="Region"
                                      value={selectedEmployee.perm_region}
                                      required
                                    />
                                    <DetailItem
                                      label="Zip Code"
                                      value={selectedEmployee.perm_zip_code}
                                      required
                                    />
                                  </div>
                                )}
                            ...
                            {selectedEmployee.onboarding_tasks?.isComplete &&
                              !checkCompleteness(selectedEmployee)
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    router.push(
                                      `/admin/employee/onboard?id=${selectedEmployee.id}&batch=${checkCompleteness(selectedEmployee).batchId}`,
                                    )
                                  }
                      ...
                      <div className="text-xs text-stone-500 space-y-1 text-right">
                        ...
                        <p>
                          * Complete profile ensures accurate record keeping
                        </p>
                      </div>
                    </div>
                  )}
...
function DetailItem({
  ...
  return (
    <div className="group">
      ...
      <p
        className={`font-medium text-base ${isEmpty ? "text-slate-300 italic" : "text-slate-800"}`}
      >
