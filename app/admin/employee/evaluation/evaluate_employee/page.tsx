      ...
      <div className="border-t border-slate-200 bg-slate-50/70 px-4 md:px-8 py-3">
        <div className="flex flex-wrap gap-3">
          {[...Array(5)].map((_, i) => (
            <Skeleton
              key={i}
              className="h-9 w-36 bg-slate-100 border border-slate-200"
            />
          ))}
        </div>
      </div>
...
function EvaluateEmployeeForm() {
  ...
  const [evaluationTemplate, setEvaluationTemplate] =
    useState<EvaluationTemplate>(DEFAULT_EVALUATION_TEMPLATE);
    ...
    const loadEvaluationTemplate = async () => {
      ...
      try {
        const res = await fetch(
          `${getApiUrl()}/api/warning-letter-templates/evaluation`,
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        router.push("/admin/employee/evaluation");
      } else {
              ...
              {failedFirstEvaluation ? (
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Button
                    ...
                    size="sm"
                    variant={
                      evaluationView === "second" ? "default" : "outline"
                    }
                    className="rounded-lg h-9 px-3 font-bold uppercase tracking-wider"
            ...
            <div className="bg-white shadow-2xl p-[60px] text-[13px] leading-relaxed text-black border border-slate-300">
              ...
              {!hasFirstBreakdown && (
                <div className="mb-6 text-xs text-blue-700 border border-blue-300 bg-blue-50 p-3">
                  Showing reconstructed per-criteria ratings based on saved
                  total score.
                </div>
              )}
