...
export default function EvaluationPage() {
  ...
  const [persistedEvaluations, setPersistedEvaluations] = useState<
  ...
  const calculateEvaluationDates = (
    dateHired: string,
    ...
    .filter((emp) => {
      const matchesSearch =
        `${emp.first_name} ${emp.last_name}`
          .toLowerCase()
          .includes(searchQuery.toLowerCase()) || emp.id.includes(searchQuery);
  ...
  const underProbationCount = employees.filter(
    (e) => getDatesForEmployee(e)?.status === "Probee",
  ).length;
  const forRecommendationCount = employees.filter(
    (e) => getDatesForEmployee(e)?.status === "For Recommendation",
  ).length;
  const regularEmployeesCount = employees.filter((e) =>
    ["Regular", "Regularized"].includes(getDatesForEmployee(e)?.status || ""),
  ).length;
  const failedEmployeesCount = employees.filter(
    (e) => getDatesForEmployee(e)?.status === "Failed",
  ).length;
        ...
        <div className="min-h-screen flex items-center justify-center p-8 text-center animate-in fade-in zoom-in-95 duration-500">
          <div className="bg-white/80 backdrop-blur-xl w-[500px] h-auto p-16 rounded-[48px] shadow-2xl shadow-slate-200/50 border border-slate-200/60 flex flex-col items-center">
            <div className="w-24 h-24 bg-rose-50 rounded-full flex items-center justify-center mb-8 group">
              <Badge
                variant="outline"
                className="h-14 w-14 border-rose-200 bg-white shadow-sm flex items-center justify-center rounded-2xl group-hover:scale-110 transition-transform duration-300"
              >
            ...
            <p className="text-slate-500 text-lg max-w-md mx-auto mb-10 leading-relaxed">
              {fetchError} Please ensure the backend server is running and try
              again.
            </p>
            <Button
              ...
              <div className="w-full px-4 md:px-8 py-3 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3">
                ...
                <div className="flex items-center gap-3 w-full lg:w-auto">
                  <Button
                    onClick={() =>
                      router.push(
                        "/admin/employee/evaluation/evaluate_employee",
                      )
                    }
                          ...
                          >
                            ...
                            <td className="px-3 py-2 text-center">
                              <Button
                                ...
                                onClick={() =>
                                  router.push(
                                    `/admin/employee/evaluation/evaluate_employee?id=${emp.id}`,
                                  )
                                }
                              >
                                View Details
