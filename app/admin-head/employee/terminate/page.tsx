...
function TerminatePageContent() {
  ...
  const [activeTab, setActiveTab] = useState<"all" | "terminated" | "rehired">(
    "all",
  );
  ...
  const prepareClearanceChecklist = async (
    ...
    try {
      ...
      if (recordRes.ok) {
        ...
        const sortedByLatest = [
          ...(sameDayMatches.length > 0 ? sameDayMatches : []),
        ]
          .filter(
      ...
      onConfirm: async () => {
        try {
          ...
          if (!employeeProfile || missingFields.length > 0) {
            ...
            router.push(
              `/admin-head/employee/onboard?id=${encodeURIComponent(employeeId)}&view=onboard&rehire=1&batch=1${rehireEmailQuery}`,
            );
            return;
          }
          ...
          if (data.success) {
            ...
            setShowDetailDialog(false);
            router.push(
              `/admin-head/employee/onboard?id=${encodeURIComponent(resolvedEmployeeId)}&view=onboard&rehire=1&batch=1${rehireEmailQuery}`,
            );
          } else {
            if (data.errors) {
              const errorMessages = Object.values(data.errors).flat().join(" ");
                  ...
                  {fetchError ? (
                    <div className="flex flex-col items-center justify-center py-24 text-center animate-in fade-in zoom-in-95 duration-500">
                      ...
                      <p className="text-slate-500 max-w-md mx-auto mb-8">
                        {fetchError} Please ensure the backend server is running
                        and try again.
                      </p>
                      <Button
                        onClick={fetchData}
                    ...
                    <>
                      <div className="bg-slate-50 border-b border-slate-100 px-6 py-4 flex items-center justify-between">
                        <div>
                          <h2 className="text-lg font-bold text-[#4A081A]">
                            Terminated History
                          </h2>
                          <p className="text-slate-400 text-xs mt-0.5">
                            {filteredTerminations.length} of{" "}
                            {baseTerminations.length} records
                          </p>
                        </div>
                      </div>
                            ...
                            <Table>
                              <TableHeader className="bg-slate-50">
                                <TableRow className="border-b border-slate-100">
                                  <TableHead className="py-4 pl-8 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                    Employee
                                  </TableHead>
                                  ...
                                  <TableHead className="py-4 pr-8 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">
                                    Action
                                  </TableHead>
                                </TableRow>
                              </TableHeader>
                            ...
                            {filteredTerminations.length > itemsPerPage && (
                              <div className="px-8 py-3 border-t border-slate-50 flex items-center justify-between bg-slate-50/30">
                                <div className="text-xs text-slate-500 font-medium">
                                  ...
                                  of {filteredTerminations.length}
                                </div>
                          ...
                          <div className="overflow-x-auto">
                            ...
                            {filteredResigned.length > itemsPerPage && (
                              <div className="px-8 py-3 border-t border-slate-50 flex items-center justify-between bg-slate-50/30">
                                <div className="text-xs text-slate-500 font-medium">
                                  Showing{" "}
                                  {(currentResignedPage - 1) * itemsPerPage + 1}{" "}
                                  to{" "}
                                  {Math.min(
                                    currentResignedPage * itemsPerPage,
                  ...
                  <div className="bg-[#FFE5EC]/20 p-4 md:px-8 border-b border-[#FFE5EC]">
                    <div className="flex justify-between items-center mb-3">
                      ...
                      <span className="text-sm font-black text-[#A4163A] bg-white px-3 py-0.5 rounded-full shadow-sm border border-[#FFE5EC]">
                        {Object.keys(completedClearanceTasks).length} /{" "}
                        {clearanceTasks.length > 0 ? clearanceTasks.length : 1}{" "}
                        Completed
                      </span>
                    </div>
                        ...
                        style={{
                          width:
                            clearanceTasks.length > 0
                              ? `${(Object.keys(completedClearanceTasks).length / clearanceTasks.length) * 100}%`
                              : "100%",
                        }}
                      />
                    </div>
                  ...
                  <Table>
                    <TableHeader className="bg-[#FFE5EC]/40">
                      <TableRow className="border-b border-[#FFE5EC] hover:bg-transparent">
                        <TableHead className="w-[200px] text-center font-black text-[#800020] uppercase tracking-[0.12em] text-[9px] py-3">
                          Completed Date
                        </TableHead>
                        <TableHead className="w-[100px] text-center font-black text-[#800020] uppercase tracking-[0.12em] text-[9px] py-3">
                          Status
                        </TableHead>
                      ...
                      {loadingTasks ? (
                        ...
                        clearanceTasks.map((task, index) => (
                          <TableRow
                            key={index}
                            className={cn(
                              "border-b border-rose-50/30 last:border-0 hover:bg-[#FFE5EC]/5 transition-colors group",
                              !isViewer && "cursor-pointer",
                            )}
                            onClick={() =>
                              !isViewer && toggleClearanceTask(task)
                            }
                          >
                                  ...
                                  className={cn(
                                    ...
                                    completedClearanceTasks[task]
                                      ? savedClearanceTasks.has(task)
                                        ? "bg-emerald-700 border-emerald-700 opacity-60 cursor-not-allowed"
                    ...
                    {!isViewer && (
                      <div className="flex gap-3">
                        <Button
                          ...
                          disabled={
                            Object.keys(completedClearanceTasks).length <
                              clearanceTasks.length || isSavingChecklist
                          }
