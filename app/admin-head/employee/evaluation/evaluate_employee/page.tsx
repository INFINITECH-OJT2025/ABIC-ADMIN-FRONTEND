...
const DEFAULT_EVALUATION_TEMPLATE = {
  ...
  criteriaOverrides: Object.fromEntries(
    CRITERIA.map((item) => [item.id, { label: item.label, desc: item.desc }]),
  ),
  agreementText:
      ...
      <div className="max-w-[850px] mx-auto bg-white shadow-sm p-[60px] border border-slate-200 space-y-10">
        ...
        <div className="space-y-8">
          {[...Array(5)].map((_, i) => (
            <div
            key={i}
            className="flex gap-4 items-start pt-4 border-t border-slate-100"
          >
...
function EvaluateEmployeeForm() {
  ...
  const [scores, setScores] =
    useState<Record<CriteriaId, string>>(EMPTY_SCORES);
    ...
    return employees
      .filter((emp) => {
        ...
        return (
          ...
          (!evalRecord ||
            (evalRecord.status !== "Regular" &&
              evalRecord.status !== "Regularized"))
        );
      })
      .sort((a, b) =>
  ...
  const showFirstEvaluationPanel =
    failedFirstEvaluation &&
    (evaluationView === "first" || evaluationView === "both");
  const showSecondEvaluationPanel =
    !failedFirstEvaluation ||
  ...
  const buildFallbackBreakdownFromTotal = (
    total?: number | null,
  ): Record<CriteriaId, number> | null => {
  ...
  const firstDisplayBreakdown = useMemo(
    () =>
      firstBreakdown ??
      buildFallbackBreakdownFromTotal(selectedEvaluation?.score_1),
    [firstBreakdown, selectedEvaluation?.score_1],
  );
  const secondDisplayBreakdown = useMemo(
    () =>
      secondBreakdown ??
      buildFallbackBreakdownFromTotal(selectedEvaluation?.score_2),
    [secondBreakdown, selectedEvaluation?.score_2],
  );

  const mapBreakdownToScoreState = (
    breakdown?: Record<CriteriaId, number> | null,
  ): Record<CriteriaId, string> => {
  ...
  useEffect(() => {
    ...
    const targetBreakdown = failedFirstEvaluation
      ? secondDisplayBreakdown
      : firstDisplayBreakdown;
    setScores(mapBreakdownToScoreState(targetBreakdown));
    const targetAgreement = failedFirstEvaluation
      ? selectedEvaluation?.agreement_2
      : selectedEvaluation?.agreement_1;
    const targetComment = failedFirstEvaluation
      ? selectedEvaluation?.comment_2
      : selectedEvaluation?.comment_1;
    const targetRatedBy = failedFirstEvaluation
      ? selectedEvaluation?.rated_by_2
      : selectedEvaluation?.rated_by;
    const targetReviewedBy = failedFirstEvaluation
      ? selectedEvaluation?.reviewed_by_2
      : selectedEvaluation?.reviewed_by;
    const targetApprovedBy = failedFirstEvaluation
      ? selectedEvaluation?.approved_by_2
      : selectedEvaluation?.approved_by;
  ...
  const currentSavedScore = selectedEvaluation?.[
    evaluationContext.targetScore as keyof Evaluation
  ...
  useEffect(() => {
    ...
    selectedEmployeeId,
    selectedEmployee,
    evaluationContext.targetScore,
    hasSavedCurrentEvaluation,
    isViewer,
  ]);
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Failed to send PDF to employee email";
      toast.error(message);
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (failedFirstEvaluation && !showSecondEvaluationPanel) {
      return toast.error(
        "Switch to 2nd Evaluation or Both to submit the second evaluation",
      );
    }
    if (Object.values(scores).some((s) => s === ""))
      return toast.error("Please complete all rating criteria");
    if (!isEditMode)
      return toast.error("Click Edit Evaluation first to modify and save");
    ...
    try {
      ...
      const derivedStatus =
        computedRemarks === "Passed"
          ? "For Recommendation"
          : evaluationContext.isSecond
            ? "Failed"
            : "Probee";
      ...
      const payload = {
        ...
        [evaluationContext.targetBreakdown]: CRITERIA.reduce(
          (acc, criterion) => {
            const criterionId = criterion.id as CriteriaId;
        ...
        [`${evaluationContext.isSecond ? "rated_by_2" : "rated_by"}`]:
          ratedBy.trim() || undefined,
        [`${evaluationContext.isSecond ? "reviewed_by_2" : "reviewed_by"}`]:
          reviewedBy.trim() || undefined,
        [`${evaluationContext.isSecond ? "approved_by_2" : "approved_by"}`]:
  ...
  const firstEvaluationPassed =
    String(firstEvaluationRemark).toLowerCase() === "passed";
  const firstFormattedComment =
    firstEvaluationRemark !== "N/A"
      ? `${firstEvaluationRemark}: ${firstEvaluationComment || "-"}`
      : "-";
  const secondFormattedComment =
    secondEvaluationRemark !== "N/A"
      ? `${secondEvaluationRemark}: ${secondEvaluationComment || "-"}`
      : "-";
  const liveComputedRemark =
    totalScore > 0 ? (totalScore >= 31 ? "Passed" : "Failed") : "";
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-10">
      ...
      {/* Action Bar */}
      <div
        className={`${showSideBySide ? "max-w-[1800px]" : "max-w-[850px]"} mx-auto mb-4 flex flex-wrap justify-between gap-2`}
      >
        <Button
          variant="outline"
          ...
          {!isViewer && (
            <>
              ...
              <Button
                ...
                disabled={
                  isSubmitting || !showSecondEvaluationPanel || !isEditMode
                }
      ...
      {failedFirstEvaluation && (
        <div
          className={`${showSideBySide ? "max-w-[1800px]" : "max-w-[850px]"} mx-auto mb-4 bg-white border border-slate-300 p-4`}
        >
      ...
      <div
        className={`${showSideBySide ? "max-w-[1800px] grid grid-cols-1 xl:grid-cols-2 gap-6" : "max-w-[850px]"} mx-auto`}
      >
          ...
          <div className="bg-white shadow-2xl p-[60px] text-[13px] leading-relaxed text-black border border-slate-300">
            ...
            <div className="space-y-2 mb-10">
              <div className="flex gap-2">
                <span className="font-bold whitespace-nowrap">
                  {evaluationTemplate.metaNameLabel}
                </span>
              ...
              <div className="flex gap-2">
                <span className="font-bold whitespace-nowrap">
                  {evaluationTemplate.metaDepartmentLabel}
                </span>
                <span className="relative inline-flex max-w-full border-b border-black pr-1">
                  <span className="text-[#D32F2F] font-bold">
                    {selectedEmployee
                      ? `${employeeDetails?.department} / ${employeeDetails?.position}`
                      : ""}
                  </span>
                </span>
              </div>
              <div className="flex gap-2">
                <span className="font-bold whitespace-nowrap">
                  {evaluationTemplate.metaRatingPeriodLabel}
                </span>
                <span className="relative inline-flex max-w-full border-b border-black pr-1">
                  <span className="text-[#D32F2F] font-bold">
                    {selectedEmployee && (
            ...
            <div className="grid grid-cols-12 mb-4 border-b-2 border-transparent relative">
              <div className="col-span-10 text-center font-bold underline">
                {evaluationTemplate.criteriaHeader}
              </div>
              <div className="col-span-2 text-center font-bold underline">
                {evaluationTemplate.ratingHeader}
              </div>
            </div>
            ...
            {!hasFirstBreakdown && (
              <div className="mb-6 text-xs text-blue-700 border border-blue-300 bg-blue-50 p-3">
                Showing reconstructed per-criteria ratings based on saved total
                score.
              </div>
            )}
              ...
              <div className="flex flex-wrap items-center gap-x-2">
                ...
                <span className="flex items-center gap-1">
                  <span className="w-3 h-3 border border-black rounded-full flex items-center justify-center">
                    {firstEvaluationAgreement === "agree" && (
                ...
                <span className="flex items-center gap-1">
                  <span className="w-3 h-3 border border-black rounded-full flex items-center justify-center">
                    {firstEvaluationAgreement === "disagree" && (
                      <div className="w-1.5 h-1.5 bg-black rounded-full" />
                    )}
                  </span>
            ...
            <div className="mb-8">
              <div className="font-bold uppercase mb-2">
                {evaluationTemplate.ratingScaleTitle}
              </div>
            ...
            <div className="mb-10">
              <div className="font-bold uppercase mb-2">
                {evaluationTemplate.interpretationTitle}
              </div>
            ...
            <div className="mb-10 font-bold flex items-center gap-6">
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`w-5 h-5 border-2 border-black flex items-center justify-center ${firstEvaluationPassed ? "bg-[#D32F2F] border-[#D32F2F]" : "bg-transparent"}`}
                >
            ...
            <div className="mb-20">
              <div className="font-bold uppercase mb-2">
                {evaluationTemplate.remarksLabel}
              </div>
              <Textarea
            ...
            <div className="grid grid-cols-2 gap-x-20 gap-y-2">
              <div className="flex gap-2">
                <span className="min-w-[80px]">
                  {evaluationTemplate.ratedByLabel}
                </span>
              ...
              <div className="flex gap-2">
                <span className="min-w-[80px]">
                  {evaluationTemplate.reviewedByLabel}
                </span>
                <div className="flex-1 border-b border-black"></div>
              </div>
              <div className="flex gap-2">
              ...
              <div className="flex gap-2">
                <span className="min-w-[80px]">
                  {evaluationTemplate.approvedByLabel}
                </span>
                <div className="flex-1 border-b border-black"></div>
              </div>
              <div className="flex gap-2">
          ...
          <div className="bg-white shadow-2xl p-[60px] text-[13px] leading-relaxed text-black border border-slate-300">
            ...
            <div className="text-center mb-10">
              <h1 className="text-[#D32F2F] font-bold text-lg uppercase tracking-tight">
                {employeeDetails?.office
                  ? employeeDetails.office.toUpperCase()
                  : "OFFICE NOT SET"}
              </h1>
              <h2 className="font-bold text-lg uppercase tracking-wider">
                {evaluationTemplate.title}
            ...
            <div className="space-y-2 mb-10">
              <div className="flex gap-2">
                ...
                <span
                  className={`${isViewDetailsMode ? "relative inline-flex max-w-full border-b border-black pr-1" : "w-full relative border-b border-black"}`}
                >
                  <span
                    className={`${isViewDetailsMode ? "text-[#D32F2F] font-bold" : "absolute left-0 -top-1 w-full text-[#D32F2F] font-bold"}`}
                  >
                    {isViewDetailsMode ? (
                  ...
                  <span className="text-[#D32F2F] font-bold">
                    {selectedEmployee && (
                      <>
                        ...
                        <span className="ml-4 text-xs italic text-slate-400 font-normal">
                          ({evaluationContext.isSecond ? "2nd" : "1st"}{" "}
                          Evaluation)
                        </span>
                      </>
                    )}
            ...
            {failedFirstEvaluation && hasSecondBreakdown && (
              <div className="mb-6 text-xs text-emerald-700 border border-emerald-300 bg-emerald-50 p-3">
                Loaded saved 2nd evaluation criteria scores for this employee.
                You can still update and save if needed.
              </div>
            )}
            {hasSavedCurrentEvaluation && !isEditMode && (
              <div className="mb-6 text-xs text-amber-800 border border-amber-300 bg-amber-50 p-3">
                This evaluation is locked to prevent accidental edits. Click{" "}
                <strong>Edit Evaluation</strong> to modify and save.
              </div>
            )}
            ...
            <div className="mb-10 text-[12px]">
              <div className="flex flex-wrap items-center gap-x-2">
                <span>{evaluationTemplate.agreementText}</span>
                <span
                  className={`flex items-center gap-1 ${isEditMode ? "cursor-pointer" : "cursor-not-allowed opacity-70"}`}
                  onClick={() => isEditMode && setAgreement("agree")}
                >
                  <span
                    className={`w-3 h-3 border border-black rounded-full flex items-center justify-center`}
                  >
                    {agreement === "agree" && (
                      <div className="w-1.5 h-1.5 bg-black rounded-full" />
                    )}
                ...
                <span
                  className={`flex items-center gap-1 ${isEditMode ? "cursor-pointer" : "cursor-not-allowed opacity-70"}`}
                  onClick={() => isEditMode && setAgreement("disagree")}
                >
                  <span
                    className={`w-3 h-3 border border-black rounded-full flex items-center justify-center`}
                  >
            ...
            <div className="mb-10 font-bold flex items-center gap-6">
              <span>{evaluationTemplate.recommendationLabel}</span>
              <div
                className={`flex items-center gap-2 ${isEditMode ? "cursor-pointer" : "cursor-not-allowed opacity-70"}`}
                onClick={() => isEditMode && setRecommendation("yes")}
              >
                <div
                  className={`w-5 h-5 border-2 border-black flex items-center justify-center ${recommendation === "yes" ? "bg-[#D32F2F] border-[#D32F2F]" : "bg-transparent"}`}
                >
                  {recommendation === "yes" && (
                    <Check className="w-4 h-4 text-white font-bold" />
                  )}
              ...
              <div
                className={`flex items-center gap-2 ${isEditMode ? "cursor-pointer" : "cursor-not-allowed opacity-70"}`}
                onClick={() => isEditMode && setRecommendation("no")}
              >
                <div
                  className={`w-5 h-5 border-2 border-black flex items-center justify-center ${recommendation === "no" ? "bg-[#D32F2F] border-[#D32F2F]" : "bg-transparent"}`}
                >
                  {recommendation === "no" && (
                    <Check className="w-4 h-4 text-white font-bold" />
                  )}
            ...
            <div className="mb-20">
              ...
              <div className="mt-2 text-xs font-semibold text-[#7B0F2B]">
                {liveComputedRemark
                  ? `${liveComputedRemark}: ${remarks.trim() || "(comment here)"}`
                  : "Status comment preview will appear once score is computed."}
              </div>
            ...
            <div className="mb-10">
              <div className="font-bold uppercase mb-2 text-[12px]">
                {evaluationTemplate.managerSignaturesTitle}
              </div>
