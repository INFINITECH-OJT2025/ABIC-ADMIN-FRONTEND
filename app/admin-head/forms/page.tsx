...
const normalizeTasks = (input: unknown): ChecklistTask[] => {
  ...
  return tasks.map((task, index) => {
    ...
    const normalizedStatus: TaskStatus =
      String(row.status).toUpperCase() === "DONE" ? "DONE" : "PENDING";
    const existingDate =
...
const isRecordDone = (record: OnboardingRecord) =>
  String(record.status).toUpperCase() === "DONE" ||
  getRecordCompletionPercentage(record) === 100;
...
export default function OnboardingChecklistPage() {
  ...
  const [employeeInfo, setEmployeeInfo] = useState<OnboardingRecord | null>(
    null,
  );
    ...
    const fetchChecklists = async () => {
      try {
        ...
        const response = await fetch(
          `${getApiUrl()}/api/onboarding-checklist`,
          {
      ...
      try {
        ...
        if (hierarchiesResponse.ok) {
          ...
          const names = Array.isArray(hierarchiesData?.data)
            ? (hierarchiesData.data as NamedOption[])
                .map((item) => item.name)
        ...
        if (departmentsResponse.ok) {
          ...
          const names = Array.isArray(departmentsData?.data)
            ? (departmentsData.data as NamedOption[])
                .map((item) => item.name)
                .filter((n): n is string => !!n)
            : [];
  ...
  const showCompletionDate =
    ...
    String(employeeInfo?.status ?? "").toUpperCase() === "DONE";
  ...
  const filteredAndSortedRecords = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      if (recordSort === "NAME_ASC")
        return a.record.name.localeCompare(b.record.name);
      if (recordSort === "NAME_DESC")
        return b.record.name.localeCompare(a.record.name);
      if (recordSort === "UPDATED_ASC")
        return withTime(a.record.updatedAt) - withTime(b.record.updatedAt);
  ...
  const persistTaskStatus = async (
    updatedTasks: ChecklistTask[],
    previousTasks: ChecklistTask[],
  ) => {
    if (!employeeInfo) return;

    try {
      const response = await fetch(
        `${getApiUrl()}/api/onboarding-checklist/${employeeInfo.id}`,
        {
          method: "PUT",
          headers: {
      ...
      setRecords((prev) =>
        prev.map((record, index) =>
          index === currentIndex ? updated : record,
        ),
      );
  ...
  const toggleTaskStatus = async (id: number, checked: boolean) => {
    ...
    const updatedTasks = tasks.map((t) =>
      t.id === id
        ? {
            ...t,
            status: (checked ? "DONE" : "PENDING") as TaskStatus,
            date: checked ? new Date().toLocaleDateString("en-CA") : "",
          }
        : t,
    );
      ...
      <div className="min-h-screen bg-slate-50/50 p-8 font-sans">
        <Card className="mx-auto mt-16 max-w-3xl rounded-[2rem] border-none shadow-2xl bg-white p-10 text-center">
          ...
          <p className="mt-2 text-slate-600">
            Preparing employee record and onboarding tasks...
          </p>
  ...
  if (error) {
    return (
      <div className="p-8 text-rose-600">
        Failed to load onboarding checklist: {error}
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50/50 pb-12 font-sans">
            ...
            <div className="flex flex-wrap items-center gap-3 text-rose-100/80 text-lg">
              ...
              <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                  <Button
                    variant="ghost"
                    className="h-auto p-0 text-white font-bold text-lg hover:bg-transparent underline underline-offset-4 decoration-rose-400"
                  >
              ...
              <div className="flex items-center gap-2">
                <Select
                  ...
                  onValueChange={(value) =>
                    setRecordStatusFilter(value as RecordStatusFilter)
                  }
                >
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="UPDATED_ASC">
                      <span className="flex items-center gap-2">
                        <History className="h-3.5 w-3.5" /> Oldest
                      </span>
                    </SelectItem>
      ...
      <main className="max-w-[1600px] mx-auto p-8 relative">
        <div className="absolute left-0 top-[25%] -translate-x-1/2 z-10">
          <Button
            onClick={handlePrev}
            disabled={records.length === 0 || currentIndex === 0}
            size="icon"
            className="rounded-full h-12 w-12 bg-white shadow-xl text-[#a0153e] border-none hover:bg-rose-50"
          >
        ...
        <div className="absolute right-0 top-[25%] translate-x-1/2 z-10">
          <Button
            onClick={handleNext}
            disabled={
              records.length === 0 || currentIndex >= records.length - 1
            }
            size="icon"
            className="rounded-full h-12 w-12 bg-white shadow-xl text-[#a0153e] border-none hover:bg-rose-50"
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-slate-100">
            <div className="p-8 bg-slate-50/30">
              <div className="flex items-center gap-2 mb-3">
                <p className="text-lg font-bold text-[#a0153e] uppercase tracking-wide">
                  Employee Name
                </p>
            ...
            <div className="p-8">
              <p className="text-lg font-bold text-[#a0153e] uppercase tracking-wide mb-3">
                Position
              </p>
              {editMode && employeeInfo ? (
                <Select
                  ...
                  onValueChange={(val) =>
                    setEmployeeInfo({ ...employeeInfo, position: val })
                  }
                >
                ...
                <p className="text-2xl md:text-3xl font-extrabold text-slate-700">
            ...
            <div className="p-8">
              ...
              {editMode && employeeInfo ? (
                <DatePicker
                  className="rounded-xl border-slate-200 h-11 w-full"
                ...
                <p className="text-2xl md:text-3xl font-extrabold text-slate-700">
                  {employeeInfo?.startDate
                    ? new Date(employeeInfo.startDate).toLocaleDateString(
                        "en-US",
            ...
            <div className="p-8">
              ...
              {editMode && employeeInfo ? (
                <Select
                  ...
                  onValueChange={(val) =>
                    setEmployeeInfo({ ...employeeInfo, department: val })
                  }
                >
                  <SelectTrigger className="rounded-xl border-slate-200 h-11">
                    <SelectValue />
        ...
        <Card className="rounded-[2rem] border-none shadow-2xl bg-white overflow-hidden">
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="border-b border-slate-100">
                <TableHead className="w-[220px] text-center font-black text-[#a0153e] uppercase tracking-widest text-[11px] py-8">
                  Date
                </TableHead>
                <TableHead className="w-[180px] text-center font-black text-[#a0153e] uppercase tracking-widest text-[11px] py-8">
                  Status
                </TableHead>
                <TableHead className="font-black text-[#a0153e] uppercase tracking-widest text-[11px] py-8">
                  Onboarding Tasks
                </TableHead>
                {editMode && (
                  <TableHead className="w-[100px] text-center font-black text-[#a0153e] uppercase tracking-widest text-[11px] py-8">
                    Action
                  </TableHead>
                )}
              </TableRow>
                    ...
                    <div className="flex justify-center items-center gap-4">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          toggleTaskStatus(item.id, !!checked)
                        }
                  ...
                  <TableCell className="py-6">
                    {editMode ? (
                      <Input
                        value={item.task}
                        onChange={(e) =>
                          updateTaskText(item.id, e.target.value)
                        }
                        className="rounded-xl border-slate-200 h-11 bg-slate-50/30"
                      />
                    ) : (
                      <span
                        className={cn(
                  ...
                  <TableCell colSpan={emptyTaskColSpan} className="py-16">
                    <div className="flex flex-col items-center justify-center text-center gap-3">
                      ...
                      <p className="text-sm text-slate-500">
                        {records.length === 0
                          ? "No onboarding records found yet. Add a record to begin."
                          : "This record has no tasks yet."}
                      </p>
                    </div>
                  </TableCell>
                </TableRow>
          ...
          <div className="bg-slate-50/50 px-10 py-8 flex justify-between items-center border-t border-slate-100">
            <div className="flex items-center gap-4">
              ...
              {editMode && (
                <Button
                  ...
                  size="sm"
                  variant="ghost"
                  className="rounded-full text-[#a0153e] font-bold bg-[#a0153e]/5 hover:bg-[#a0153e]/10 px-6"
                >
            ...
            <div className="flex gap-4">
              <Button
                variant="outline"
                className="rounded-full px-8 h-12 shadow-sm border-slate-200 font-bold text-slate-600 hover:bg-slate-100 transition-colors"
              >
      ...
      <Dialog open={addRecordOpen} onOpenChange={setAddRecordOpen}>
        <DialogContent className="sm:max-w-[560px] border-2 border-[#C9184A] p-0 overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            <div>
              <p className="text-[11px] font-bold text-[#a0153e] uppercase tracking-[0.2em] mb-2">
                Employee Name
              </p>
              <Input
          ...
          <DialogFooter className="px-6 pb-6 gap-2">
            ...
            <Button
              ...
              className="rounded-full bg-[#a0153e] hover:bg-[#801030] text-white"
            >
      ...
      >
        <AlertDialogContent className="border-2 border-rose-200">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setTaskIdToDelete(null)}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
